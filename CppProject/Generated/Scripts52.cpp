...
namespace CppProject
{
	void render_world_tl_reset()
	{
		...
		render_set_uniform_int(/*"uGlintEnabled"*/ STR(2501), IntType(0));
	...
	{
		StringType fn = global::load_folder + /*"/"*/ STR(19) + sVar(filename);
		debug({ /*"Loading "*/ STR(2506) + DsList(global::res_type_name_list).Value(sVar(type)), fn });
		switch ((IntType)sVar(type))
		{
			...
			case e_res_type_BLOCK_SHEET:
			{
				...
				if (sVar(type) == e_res_type_LEGACY_BLOCK_SHEET)
				{
					...
					if (global::load_folder == global::save_folder)
						sVar(filename) = filename_new_ext(filename_name(fn), /*"_converted"*/ STR(2507) + filename_ext(fn));
					texture_export(sInt(block_sheet_texture), global::save_folder + /*"/"*/ STR(19) + sVar(filename));
			...
			{
				StringType cachefn = fn + /*".meshcache"*/ STR(2508);
			...
			case e_res_type_TEXTURE:
			{
				if (sVar(texture) > 0)
					texture_free(sVar(texture));
				sVar(texture) = null_;
				if (filename_ext(fn) == /*".dat"*/ STR(712))
				{
					IntType map = new_minecraft_map(fn);
					if (!(map > 0))
					{
						error(/*"errorloadmap"*/ STR(2509));
						break;
					}
					else
	...
	{
		StringType fname = global::load_folder + /*"/"*/ STR(19) + sVar(filename);
		switch (StringType(sStr(load_stage)).id)
		{
			case 1277: // unzip
			{
				debug({ /*"res_load_pack"*/ STR(2516), /*"unzip"*/ STR(1277) });
				if (sVar(type) != e_res_type_PACK_UNZIPPED)
				{
					if (!unzip(fname))
					{
						log({ /*"Error unzipping pack"*/ STR(2517) });
						error(/*"errorunzippack"*/ STR(2518));
						withOne (app, global::_app->id, self->id)
				...
				sStr(load_stage) = /*"modeltextures"*/ STR(1359);
				global::load_assets_dir = unzip_directory;
				res_load_pack_version(self);
				withOne (app, global::_app->id, self->id)
				{
					ObjType(obj_popup, global::_app->popup_loading)->text = text_get({ /*"loadpackmodeltextures"*/ STR(2519) });
			...
			{
				debug({ /*"res_load_pack"*/ STR(2516), /*"modeltextures"*/ STR(1359) });
				res_load_pack_model_textures(self);
				sStr(load_stage) = /*"blocktextures"*/ STR(2520);
				withOne (app, global::_app->id, self->id)
				{
					ObjType(obj_popup, global::_app->popup_loading)->text = text_get({ /*"loadpackblocktextures"*/ STR(2521) });
			...
			case 2520: // blocktextures
			{
				debug({ /*"res_load_pack"*/ STR(2516), /*"blocktextures"*/ STR(2520) });
				file_rename_lib(global::load_assets_dir + mc_textures_directory + /*"blocks"*/ STR(996), global::load_assets_dir + mc_textures_directory + /*"block"*/ STR(4));
				res_load_pack_block_textures(self);
				sStr(load_stage) = /*"itemtextures"*/ STR(2522);
				withOne (app, global::_app->id, self->id)
				{
					ObjType(obj_popup, global::_app->popup_loading)->text = text_get({ /*"loadpackitemtextures"*/ STR(2523) });
			...
			case 2522: // itemtextures
			{
				debug({ /*"res_load_pack"*/ STR(2516), /*"itemtextures"*/ STR(2522) });
				file_rename_lib(global::load_assets_dir + mc_textures_directory + /*"items"*/ STR(2524), global::load_assets_dir + mc_textures_directory + /*"item"*/ STR(523));
				res_load_pack_item_textures(self, /*"diffuse"*/ STR(609), /*""*/ STR(0));
				res_load_pack_item_textures(self, /*"material"*/ STR(614), /*"_s"*/ STR(2525));
				res_load_pack_item_textures(self, /*"normal"*/ STR(603), /*"_n"*/ STR(2526));
				res_load_pack_particle_textures(self);
				res_load_pack_misc(self);
				res_update_colors(self);
				sBool(ready) = true;
				global::_app->history_resource_update = true;
				log({ /*"Pack loaded"*/ STR(2527) });
