...
namespace CppProject
{
	void menu_draw(ScopeAny self)
	{
		...
		for (IntType i = IntType(0); i < ds_list_size(sInt(menu_list)); i++)
		{
			...
			if (ObjType(obj_menu, m)->menu_window != window_get_current())
				continue;
			if (ObjType(obj_menu, m)->menu_ani_type == /*"hide"*/ STR(722))
			{
				...
				if (ObjType(obj_menu, m)->menu_ani_type == /*"show"*/ STR(723))
				{
					ObjType(obj_menu, m)->menu_ani += test_reduced_motion(IntType(1), (0.1 * global::delta));
			...
			if (ObjType(obj_menu, m)->menu_steps == IntType(0))
				draw_set_alpha(0.0);
			ObjType(obj_menu, m)->menu_ani_ease = ease(((ObjType(obj_menu, m)->menu_ani_type == /*"show"*/ STR(723)) ? /*"easeoutexpo"*/ STR(568) : /*"easeinexpo"*/ STR(567)), ObjType(obj_menu, m)->menu_ani);
			...
			if (sVar(window_busy) == /*"menu"*/ STR(1058) && ObjType(obj_menu, m)->menu_ani_type != /*"hide"*/ STR(722) && menu_active)
			...
			if (keyboard_check_pressed(vk_enter) || keyboard_check_pressed(vk_escape))
			{
				if (ObjType(obj_menu, m)->menu_nav_use)
				{
					...
					ObjType(obj_menu, m)->menu_ani_type = /*"hide"*/ STR(722);
			...
			if ((!(ObjType(obj_scrollbar, ObjType(obj_menu, m)->menu_scroll_vertical)->needed && ObjType(obj_scrollbar, ObjType(obj_menu, m)->menu_scroll_vertical)->mouseon) && !(ObjType(obj_scrollbar, ObjType(obj_menu, m)->menu_scroll_horizontal)->needed && ObjType(obj_scrollbar, ObjType(obj_menu, m)->menu_scroll_horizontal)->mouseon) && sBool(mouse_left_released) && menu_active && ObjType(obj_menu, m)->menu_ani_type != /*"hide"*/ STR(722) && sVar(menu_search_busy) == /*""*/ STR(0)) || nav_close)
			{
			...
			if (sVar(window_busy) == /*""*/ STR(0) && ObjType(obj_menu, m)->menu_ani_type != /*"hide"*/ STR(722) && menu_active)
				sVar(window_busy) = /*"menu"*/ STR(1058);
		}
	...
	void menu_quality_settings(ScopeAny self)
	{
		...
		VarType text;
		draw_set_font(sInt(font_label));
		capwid = max({ IntType(176), text_max_width({ /*"viewmodepass"*/ STR(1467) }) + IntType(16) });
		text = text_get({ /*"viewmodepass"*/ STR(1467) + DsList(global::render_pass_list).Value(sInt(project_render_pass)) });
		tab_control_menu(self);
		draw_button_menu(self, { /*"viewmodepass"*/ STR(1467), e_menu_LIST, sVar(dx), sVar(dy), sVar(dw), IntType(24), sInt(project_render_pass), text, ID_action_project_render_pass });
		tab_next(self);
	...
	RealType menu_settings_draw(ScopeAny self)
	{
		if (sInt(settings_menu_window) != window_get_current())
			return IntType(0);
		if (sStr(settings_menu_ani_type) == /*"hide"*/ STR(722))
		{
			...
			if (sStr(settings_menu_ani_type) == /*"show"*/ STR(723))
			{
				if (sReal(settings_menu_quick) > 0)
		...
		if (sStr(settings_menu_name) == /*""*/ STR(0))
			return IntType(0);
		VarType settingsmenuease = ease(((sStr(settings_menu_ani_type) == /*"show"*/ STR(723)) ? /*"easeoutexpo"*/ STR(568) : /*"easeinexpo"*/ STR(567)), sVar(settings_menu_ani));
		...
		if (sVar(window_busy) == /*"settingsmenu"*/ STR(1024) && sStr(settings_menu_ani_type) == /*""*/ STR(0) && sVar(settings_menu_ani) == IntType(1))
			sVar(window_busy) = /*""*/ STR(0);
		sBool(content_mouseon) = app_mouse_box(self, sVar(content_x), sVar(content_y), sVar(content_width), sVar(content_height));
		...
		if (sStr(settings_menu_ani_type) == /*""*/ STR(0) && sBool(mouse_left_released) && !sBool(context_menu_mouseon) && (ds_list_size(sInt(menu_list)) == IntType(0)) && (sVar(window_focus) == /*""*/ STR(0) || sVar(window_focus) == string(sInt(settings_menu_scroll))))
		{
			if (sInt(settings_menu_script) > 0)
			{
				...
				{
					sVar(settings_menu_ani) = IntType(1);
					sStr(settings_menu_ani_type) = /*"hide"*/ STR(722);
					sVar(window_busy) = /*"settingsmenu"*/ STR(1024);
		...
		if (sVar(window_busy) == /*""*/ STR(0) && sStr(settings_menu_ani_type) != /*"hide"*/ STR(722))
			sVar(window_busy) = /*"settingsmenu"*/ STR(1024);
		return 0.0;
	}
	...
	void menu_settings_set(ScopeAny self, VarType xx, VarType yy, StringType name, RealType buttonheight)
	{
		...
		sVar(settings_menu_ani) = IntType(0);
		sStr(settings_menu_ani_type) = /*"show"*/ STR(723);
	...
	BoolType menu_swatches_draw(ScopeAny self)
	{
		...
		for (IntType s = IntType(0); s < array_length(VarType::CreateRef(global::minecraft_swatch_array)); s++)
		{
			...
			sVar(dy) += IntType(14);
			draw_label(self, { text_get({ /*"swatch"*/ STR(1664) + ObjType(obj_swatch, swatch)->name }), sVar(dx), sVar(dy), fa_left, fa_bottom, global::c_text_tertiary, global::a_text_tertiary, sInt(font_subheading) });
			sVar(dy) += IntType(9);
			VarType xx;
			xx = sVar(dx);
			for (IntType c = IntType(0); c < array_length(VarType::CreateRef(ObjType(obj_swatch, swatch)->colors)); c++)
			{
				if (draw_button_swatch(self, xx, sVar(dy), IntType(20), IntType(20), /*"swatch"*/ STR(1664) + ObjType(obj_swatch, swatch)->name + ObjType(obj_swatch, swatch)->color_names.Value(c), ObjType(obj_swatch, swatch)->colors.Value(c)))
				{
					if (ObjType(obj_context_menu_level, sVar(context_menu_current))->name == /*"contextmenuswatchset"*/ STR(1514))
	...
	VarType menu_transitions(ScopeAny self, VarType xx, VarType yy, VarType wid, VarType hei)
	{
		...
		if (sVar(context_menu_name) != /*""*/ STR(0) && transition != null_)
		{
			...
			if (transition != null_)
			{
				...
				ObjType(obj_menu, sVar(menu_current))->menu_ani_type = /*"hide"*/ STR(722);
				sVar(window_busy) = /*""*/ STR(0);
				return true;
			}
	...
	VarType minecraft_asset_get_name(StringType type, VarType name)
	{
		...
		if (!text_exists(type + name))
			return (dev_mode_debug_names ? /*"?????? ["*/ STR(1669) + name + /*"]"*/ STR(1213) : string_format_snakecase(name));
		else
			return text_get({ type + name }) + (dev_mode_debug_names ? /*" ["*/ STR(1670) + name + /*"]"*/ STR(1213) : /*""*/ STR(0));
		...
		withOne (obj_minecraft_assets, global::mc_assets, noone)
		{
			switch (StringType(global::load_assets_stage).id)
			{
				case 1277: // unzip
				{
					...
					if (array_length(VarType::CreateRef(file_find(global::load_assets_dir + mc_character_directory, /*".mimodel"*/ STR(698)))) == IntType(0) || array_length(VarType::CreateRef(file_find(global::load_assets_dir + mc_block_directory, /*".json"*/ STR(699)))) == IntType(0) || array_length(VarType::CreateRef(file_find(global::load_assets_dir + mc_textures_directory + /*"block/"*/ STR(793), /*".png"*/ STR(10)))) == IntType(0))
						exists = false;
					if (!(exists > 0))
					{
						debug_timer_start();
						...
						{
							log({ /*"Could not unzip archive"*/ STR(1671) });
							directory_delete_lib(global::load_assets_dir);
							access_error();
							return false;
						}
						debug_timer_stop(/*"Unzip archive"*/ STR(1672));
					}
					else
						log({ /*"Archive already unzipped, re-using"*/ STR(1673), global::load_assets_dir + mc_assets_directory });
					
					global::load_assets_stage = /*"biomes"*/ STR(1674);
				...
				case 1674: // biomes
				{
					withOne (obj_resource, global::mc_res, self->id)
						minecraft_assets_load_biomes(global::biome_list, DsMap(global::load_assets_map).Value(/*"biomes"*/ STR(1674)));
					...
					global::load_assets_stage = /*"textures"*/ STR(814);
				...
				case 814: // textures
				{
					VarType modeltextureslist = DsMap(global::load_assets_map).Value(/*"model_textures"*/ STR(1675));
					if (is_undefined(modeltextureslist))
					{
						log({ /*"No model textures found"*/ STR(1676) });
						return false;
					}
					ds_list_copy((IntType)(self->model_texture_list), (IntType)(modeltextureslist));
					VarType blocktextureslist = DsMap(global::load_assets_map).Value(/*"block_textures"*/ STR(1677));
					if (is_undefined(blocktextureslist))
					{
						log({ /*"No block textures found"*/ STR(1678) });
						return false;
					}
					ds_list_copy((IntType)(self->block_texture_list), (IntType)(blocktextureslist));
					VarType blocktexturesanimatedlist = DsMap(global::load_assets_map).Value(/*"block_textures_animated"*/ STR(1679));
					if (is_undefined(blocktexturesanimatedlist))
					{
						log({ /*"No animated block textures found"*/ STR(1680) });
						return false;
					}
					ds_list_copy((IntType)(self->block_texture_ani_list), (IntType)(blocktexturesanimatedlist));
					VarType blocktexturescolorlist = DsMap(global::load_assets_map).Value(/*"block_textures_color"*/ STR(1681));
					if (is_undefined(blocktexturescolorlist))
					{
						log({ /*"No block texture colors found"*/ STR(1682) });
						return false;
					}
					...
					{
						if (string_char_at(DsMap(self->block_texture_color_map).Value(key), IntType(1)) == /*"#"*/ STR(831))
					...
					VarType blocktexturepreview = DsMap(global::load_assets_map).Value(/*"block_textures_preview"*/ STR(1683));
					if (!is_undefined(blocktexturepreview))
						ds_map_copy((IntType)(self->block_texture_preview_map), (IntType)(blocktexturepreview));
					VarType itemtextureslist = DsMap(global::load_assets_map).Value(/*"item_textures"*/ STR(1684));
					if (is_undefined(itemtextureslist))
					{
						log({ /*"No item textures found"*/ STR(1685) });
						return false;
					}
					ds_list_copy((IntType)(self->item_texture_list), (IntType)(itemtextureslist));
					VarType particletextureslist = DsMap(global::load_assets_map).Value(/*"particle_textures"*/ STR(1686));
					if (is_undefined(particletextureslist))
					{
						log({ /*"No particle textures found"*/ STR(1687) });
						return false;
					}
					if (is_undefined(DsMap(global::load_assets_map).Value(/*"particles"*/ STR(525))))
					{
						log({ /*"No particle list found"*/ STR(1688) });
						return false;
					}
					...
					withOne (obj_resource, global::mc_res, self->id)
					{
						...
						res_load_pack_item_textures(ScopeAny(self), /*"diffuse"*/ STR(609), /*""*/ STR(0));
						...
						minecraft_assets_load_particles(DsMap(global::load_assets_map).Value(/*"particles"*/ STR(525)));
					...
					global::load_assets_stage = /*"misc"*/ STR(1689);
				...
				case 1689: // misc
				{
					if (is_undefined(DsMap(global::load_assets_map).Value(/*"patterns"*/ STR(1690))))
					{
						log({ /*"No pattern designs list found"*/ STR(1691) });
						return false;
					}
					VarType patternlist = DsMap(global::load_assets_map).Value(/*"patterns"*/ STR(1690));
					...
					if (is_undefined(DsMap(global::load_assets_map).Value(/*"sherds"*/ STR(939))))
					{
						log({ /*"No sherd keys found"*/ STR(1692) });
						return false;
					}
					else
						ds_map_copy((IntType)(global::minecraft_sherd_map), (IntType)(DsMap(global::load_assets_map).Value(/*"sherds"*/ STR(939))));
					
					if (is_undefined(DsMap(global::load_assets_map).Value(/*"armor"*/ STR(753))))
					{
						log({ /*"No armor data found"*/ STR(1693) });
						return false;
					}
					else
					{
						VarType armormap = DsMap(global::load_assets_map).Value(/*"armor"*/ STR(753));
						ds_list_copy((IntType)(global::minecraft_armor_trim_pattern_list), (IntType)(DsMap(armormap).Value(/*"pattern"*/ STR(1694))));
						ds_list_copy((IntType)(global::minecraft_armor_trim_material_list), (IntType)(DsMap(armormap).Value(/*"material"*/ STR(614))));
					}
					
					if (is_undefined(DsMap(global::load_assets_map).Value(/*"map_colors"*/ STR(1695))))
					{
						log({ /*"No map colors found"*/ STR(1696) });
						return false;
					}
					VarType colors = DsMap(global::load_assets_map).Value(/*"map_colors"*/ STR(1695));
					...
					global::load_assets_stage = /*"models"*/ STR(1697);
					global::load_assets_progress = 0.5;
					VarType swatches = DsMap(global::load_assets_map).Value(/*"swatches"*/ STR(1698));
					if (is_undefined(swatches))
					{
						log({ /*"No swatches found"*/ STR(1699) });
						return false;
					}
					for (IntType i = IntType(0); i < ds_list_size((IntType)(swatches)); i++)
					{
						...
						ObjType(obj_swatch, s)->name = DsMap(swatch).Value(/*"name"*/ STR(791));
						...
						VarType colors = DsMap(swatch).Value(/*"colors"*/ STR(1700));
						for (IntType j = IntType(0); j < ds_list_size((IntType)(colors)); j++)
						{
							VarType c = DsList(colors).Value(j);
							array_add(VarType::CreateRef(ObjType(obj_swatch, s)->color_names), DsMap(c).Value(/*"name"*/ STR(791)));
							array_add(VarType::CreateRef(ObjType(obj_swatch, s)->colors), hex_to_color(DsMap(c).Value(/*"value"*/ STR(801))));
							ds_map_add(ObjType(obj_swatch, s)->map, DsMap(c).Value(/*"name"*/ STR(791)), hex_to_color(DsMap(c).Value(/*"value"*/ STR(801))));
							ds_map_add(global::minecraft_swatch_color_map, DsMap(swatch).Value(/*"name"*/ STR(791)) + /*":"*/ STR(760) + DsMap(c).Value(/*"name"*/ STR(791)), hex_to_color(DsMap(c).Value(/*"value"*/ STR(801))));
						}
						ObjType(obj_swatch, s)->size = array_length(VarType::CreateRef(ObjType(obj_swatch, s)->colors));
					}
					global::minecraft_swatch_dyes = minecraft_get_swatch(/*"dye"*/ STR(1701));
					if (global::minecraft_swatch_dyes == null_)
					{
						log({ /*"No dyes found"*/ STR(1702) });
						return false;
					}
					break;
				}
				...
				case 1697: // models
				{
					VarType characterslist = DsMap(global::load_assets_map).Value(/*"characters"*/ STR(1703));
					if (is_undefined(characterslist))
					{
						log({ /*"No character list found"*/ STR(1704) });
						return false;
					}
					debug_timer_start();
					for (IntType i = IntType(0); i < ds_list_size((IntType)(characterslist)); i++)
					{
						...
						if (!(model > 0))
						{
							log({ /*"Could not load model"*/ STR(1705) });
							continue;
						}
						DsMap(self->model_name_map)[ObjType(obj_model, model)->name] = model;
						ds_list_add({ self->char_list, model });
					}
					debug_timer_stop(/*"Load characters"*/ STR(1706));
					VarType specialblockslist = DsMap(global::load_assets_map).Value(/*"special_blocks"*/ STR(1707));
					if (is_undefined(specialblockslist))
					{
						log({ /*"No special block list found"*/ STR(1708) });
						return false;
					}
					debug_timer_start();
					...
					debug_timer_stop(/*"Load special blocks"*/ STR(1709));
					debug_timer_start();
					global::load_assets_stage = /*"blocks"*/ STR(996);
				...
				case 996: // blocks
				{
					VarType blockslist = DsMap(global::load_assets_map).Value(/*"blocks"*/ STR(996));
					if (is_undefined(blockslist))
					{
						log({ /*"No block list found"*/ STR(1710) });
						return false;
					}
					for (IntType _it = 0, _it_max = IntType(20); _it < _it_max; _it++)
					{
						...
						{
							global::load_assets_stage = /*"done"*/ STR(1711);
							break;
						}
						VarType blockmap;
						IntType block;
						...
						if (!(block > 0))
						{
							log({ /*"Could not load block"*/ STR(1712) });
							continue;
						}
					...
					{
						DsMap(self->block_name_map)[/*"flowing_water"*/ STR(1713)] = DsMap(self->block_name_map).Value(/*"water"*/ STR(789));
						DsMap(self->block_name_map)[/*"flowing_lava"*/ STR(1714)] = DsMap(self->block_name_map).Value(/*"lava"*/ STR(1715));
						DsMap(self->block_liquid_slot_map)[/*"water"*/ STR(789)] = ds_list_find_index((IntType)(self->block_texture_ani_list), /*"block/water_still"*/ STR(1716));
						DsMap(self->block_liquid_slot_map)[/*"lava"*/ STR(1715)] = ds_list_find_index((IntType)(self->block_texture_ani_list), /*"block/lava_still"*/ STR(1717));
						DsMap(self->block_liquid_slot_map)[/*"flowing_water"*/ STR(1713)] = ds_list_find_index((IntType)(self->block_texture_ani_list), /*"block/water_flow"*/ STR(1718));
						DsMap(self->block_liquid_slot_map)[/*"flowing_lava"*/ STR(1714)] = ds_list_find_index((IntType)(self->block_texture_ani_list), /*"block/lava_flow"*/ STR(1719));
						if (!dev_mode_skip_blocks)
							DsMap(self->block_id_map)[/*"minecraft:grass_path"*/ STR(1720)] = DsMap(self->block_id_map).Value(/*"minecraft:dirt_path"*/ STR(1721));
						...
						while (!is_undefined(key))
						{
							...
							if (!is_undefined(newid))
								newidnomc = string_replace(DsMap(curmap).Value(/*"id"*/ STR(86)), /*"minecraft:"*/ STR(810), /*""*/ STR(0));
							else
								newidnomc = /*""*/ STR(0);
							...
							if (!is_undefined(DsMap(curmap).Value(/*"data"*/ STR(1722))))
								minecraft_assets_load_legacy_block_data(self, curid, DsMap(curmap).Value(/*"data"*/ STR(1722)), IntType(0), IntType(1));
							for (IntType d = IntType(0); d < IntType(16); d++)
						...
						debug_timer_stop(/*"Load blocks"*/ STR(1723));
						...
						{
							ArrType filesarr = file_find(global::load_assets_dir + mc_blockstates_directory, /*".json"*/ STR(699));
							...
							if (ds_list_size(unusedlist) > IntType(0))
							{
								...
								for (IntType i = IntType(0); i < ds_list_size(unusedlist); i++)
									str += /*"  "*/ STR(1725) + filename_name(DsList(unusedlist).Value(i)) + /*"\n"*/ STR(945);
								log({ str });
							}
							filesarr = file_find(global::load_assets_dir + mc_models_directory + /*"block/"*/ STR(793), /*".json"*/ STR(699));
						...
						log({ /*"Loaded assets successfully"*/ STR(1727) });
