...
namespace CppProject
{
	...
	RealType language_add(ScopeAny self)
	{
		...
		if (file_exists_lib(languages_directory + filename_name(fn)))
			if (!question(/*"This file already exists the language file directory. Do you want to replace the file?"*/ STR(1230)))
				return IntType(0);
	...
	{
		log({ /*"Loading language file"*/ STR(1231), fn });
		ds_map_clear(map);
		if (filename_ext(fn) == /*".milanguage"*/ STR(1232))
		{
			StringType convfn = file_directory + /*"conv.tmp"*/ STR(1233);
			json_file_convert_unicode(fn, convfn);
			if (!file_exists_lib(convfn))
			{
				StringType msg = /*"An error occurred while reading the language file:\nCould not convert."*/ STR(1234);
				log({ msg });
				window_set_caption(/*"Error"*/ STR(1235));
				show_message(msg);
			...
			{
				StringType msg = /*"An error occured while reading the language file:\n"*/ STR(1236) + global::json_error + /*" on line "*/ STR(1201) + string(global::json_line) + /*", column "*/ STR(1202) + string(global::json_column);
				log({ msg });
				window_set_caption(/*"Error"*/ STR(1235));
				show_message(msg);
				window_set_caption(/*""*/ STR(0));
	...
	RealType language_load_map(StringType pre, VarType smap, IntType dmap)
	{
		...
		while (!is_undefined(key))
		{
			if (string_contains(key, /*"/"*/ STR(19)))
				language_load_map(pre + string_replace(key, /*"/"*/ STR(19), /*""*/ STR(0)), DsMap(smap).Value(key), dmap);
			else
	...
	RealType languages_load()
	{
		...
		if (!file_exists_lib(fn))
			return IntType(0);
		log({ /*"Loading languages"*/ STR(1239), fn });
		...
		VarType list, obj;
		list = DsMap(map).Value(/*"languages"*/ STR(1241));
		for (IntType i = IntType(0); i < ds_list_size((IntType)(list)); i++)
		{
			...
			locale = value_get_string(DsMap(map).Value(/*"locale"*/ STR(1242)), /*""*/ STR(0));
			fn = value_get_string(DsMap(map).Value(/*"filename"*/ STR(1243)), /*""*/ STR(0));
			language_remove(fn);
			IntType obj = (new obj_language)->id;
	...
	void languages_save()
	{
		log({ /*"Saving languages"*/ STR(1244), languages_file });
		json_save_start(languages_file);
		json_save_object_start();
		json_save_var(/*"format"*/ STR(92), settings_format);
		json_save_array_start({ /*"languages"*/ STR(1241) });
		withAll (obj_language, noone)
		{
			json_save_object_start();
			json_save_var(/*"name"*/ STR(791), json_string_encode(self->name));
			json_save_var(/*"locale"*/ STR(1242), self->locale);
			json_save_var(/*"filename"*/ STR(1243), self->filename);
			json_save_object_done();
		}
			...
			case 1342: // particleeditortypespritetemplate
			{
				for (IntType i = IntType(0); i < ds_list_size(global::particle_template_list); i++)
				{
					...
					if (ObjType(obj_particle_template, temp)->animated)
						menu_add_item({ ObjType(obj_particle_template, temp)->name, text_get({ /*"particleeditortypespritetemplate"*/ STR(1342) + ObjType(obj_particle_template, temp)->name }) + /*" "*/ STR(20) + text_get({ /*"particleeditortypespritetemplateframes"*/ STR(1343), ObjType(obj_particle_template, temp)->frames }) });
					else
			...
			case 1431: // frameeditorcameravideosize
			{
				...
				for (IntType i = IntType(0); i < ds_list_size(global::videotemplate_list); i++)
					withOne (Object, DsList(global::videotemplate_list).Value(i), self->id)
						menu_add_item({ self->id, text_get({ /*"projectvideosizetemplate"*/ STR(1433) + idVar(self->id, name) }) + /*" ("*/ STR(1188) + string(sVar(width)) + /*"x"*/ STR(822) + string(sVar(height)) + /*")"*/ STR(1088) });
