...
namespace CppProject
{
	...
	VarType text_control_name(VarType keybind)
	{
		...
		switch ((IntType)keybind.Value(e_keybind_key_CHAR))
		{
			case vk_left:
			{
				char_ = text_get({ /*"keyleft"*/ STR(3752) });
				break;
			}
			case vk_right:
			{
				char_ = text_get({ /*"keyright"*/ STR(3753) });
				break;
			}
			case vk_up:
			{
				char_ = text_get({ /*"keyup"*/ STR(3754) });
				break;
			}
			case vk_down:
			{
				char_ = text_get({ /*"keydown"*/ STR(3755) });
				break;
			}
			case vk_enter:
			{
				char_ = text_get({ /*"keyenter"*/ STR(3756) });
				break;
			}
			case vk_escape:
			{
				char_ = text_get({ /*"keyescape"*/ STR(3757) });
				break;
			}
			case vk_space:
			{
				char_ = text_get({ /*"keyspace"*/ STR(3758) });
				break;
			}
			case vk_lshift:
			{
				char_ = text_get({ /*"keyleftshift"*/ STR(3759) });
				break;
			}
			case vk_rshift:
			{
				char_ = text_get({ /*"keyrightshift"*/ STR(3760) });
				break;
			}
			case vk_lalt:
			{
				char_ = text_get({ /*"keyleftalt"*/ STR(3761) });
				break;
			}
			case vk_ralt:
			{
				char_ = text_get({ /*"keyrightalt"*/ STR(3762) });
				break;
			}
			case vk_lcontrol:
			{
				char_ = text_get({ /*"keyleftcontrol"*/ STR(3763) });
				break;
			}
			case vk_rcontrol:
			{
				char_ = text_get({ /*"keyrightcontrol"*/ STR(3764) });
				break;
			}
			case vk_backspace:
			{
				char_ = text_get({ /*"keybackspace"*/ STR(3765) });
				break;
			}
			case vk_tab:
			{
				char_ = text_get({ /*"keytab"*/ STR(3766) });
				break;
			}
			case vk_home:
			{
				char_ = text_get({ /*"keyhome"*/ STR(3767) });
				break;
			}
			case vk_end:
			{
				char_ = text_get({ /*"keyend"*/ STR(3768) });
				break;
			}
			case vk_delete:
			{
				char_ = text_get({ /*"keydelete"*/ STR(3769) });
				break;
			}
			case vk_insert:
			{
				char_ = text_get({ /*"keyinsert"*/ STR(3770) });
				break;
			}
			case vk_pageup:
			{
				char_ = text_get({ /*"keypageup"*/ STR(3771) });
				break;
			}
			case vk_pagedown:
			{
				char_ = text_get({ /*"keypagedown"*/ STR(3772) });
				break;
			}
			case vk_pause:
			{
				char_ = text_get({ /*"keypause"*/ STR(3773) });
				break;
			}
			case vk_printscreen:
			{
				char_ = text_get({ /*"keyprintscreen"*/ STR(3774) });
				break;
			}
			case vk_f1:
			{
		...
		if (keybind.Value(e_keybind_key_ALT) > 0)
			text = text_get({ /*"keyalt"*/ STR(3787) }) + (text != /*""*/ STR(0) ? (/*" + "*/ STR(2923) + text) : /*""*/ STR(0));
		if (keybind.Value(e_keybind_key_CTRL) > 0)
			text = text_get({ /*"keycontrol"*/ STR(3788) }) + (text != /*""*/ STR(0) ? (/*" + "*/ STR(2923) + text) : /*""*/ STR(0));
		if (keybind.Value(e_keybind_key_SHIFT) > 0)
			text = text_get({ /*"keyshift"*/ STR(3789) }) + (text != /*""*/ STR(0) ? (/*" + "*/ STR(2923) + text) : /*""*/ STR(0));
		return text;
	}
	...
	VarType text_get(VarArgs argument)
	{
		...
		{
			if (!text_exists(key))
				return /*"<No text found for \""*/ STR(3790) + key + /*"\">"*/ STR(3791);
	...
	IntType text_max_width(VarArgs argument)
	{
		...
		IntType wid = IntType(0);
		for (IntType a = IntType(0); a < argument_count; a++)
			wid = max({ wid, string_width(text_get({ argument[a] }) + /*":"*/ STR(760)) });
		return wid;
	}
	...
	BoolType textbox_draw(ScopeAny self, VarType tbx, VarType xx, VarType yy, VarType w, VarType h, BoolType contextmenu, BoolType right)
	{
		...
		inserttext = /*""*/ STR(0);
		lineheight = string_height(/*" "*/ STR(20));
		mouseover = (sBool(content_mouseon) && app_mouse_box(self, xx, yy, w, h));
		if (!sBool(mouse_left) && ((sVar(window_busy) == string(tbx) + /*"tbxrelease"*/ STR(3793)) || (sVar(window_busy) == string(tbx) + /*"click"*/ STR(1062))))
		...
		if (sVar(window_focus) == string(tbx))
		{
				...
				if (key_press.Value(vk_right) || key_press.Value(vk_left))
				{
					if (keyboard_check(vk_control))
					{
						if (key_press.Value(vk_right))
						{
							...
							{
								char_ = string_char_at(idVar(tbx, text), (IntType)(sVar(textbox_select_mousepos) + IntType(1)));
								if (char_ == /*""*/ STR(0) || char_ == /*" "*/ STR(20) || char_ == /*"\n"*/ STR(945))
									break;
						...
						else
						{
							...
							if (char_ == /*" "*/ STR(20) || char_ == /*"\n"*/ STR(945) || sVar(textbox_select_mousepos) == IntType(0))
							{
								sVar(textbox_select_mousepos)--;
							...
							{
								char_ = string_char_at(idVar(tbx, text), (IntType)(sVar(textbox_select_mousepos)));
								if (char_ == /*""*/ STR(0) || char_ == /*" "*/ STR(20) || char_ == /*"\n"*/ STR(945) || (idArr(tbx, line).Value(sVar(textbox_select_mouseline)) != /*""*/ STR(0) && sVar(textbox_select_mousepos) == IntType(0)))
									break;
			...
			if (!sBool(mouse_left) && (sVar(window_busy) == string(tbx) + /*"click"*/ STR(1062)))
				sVar(window_busy) = /*""*/ STR(0);
			if (!sBool(mouse_left) && (sVar(window_busy) == string(tbx)))
				sVar(window_busy) = string(tbx) + /*"tbxrelease"*/ STR(3793);
		...
		else
		{
			if (changetext || idVar(tbx, last_width) != w || inserttext != /*""*/ STR(0) || deletetext != IntType(0))
			{
				for (l = IntType(1); l < idVar(tbx, lines); l++)
				{
					...
					{
						p = string_pos(/*" "*/ STR(20), idArr(tbx, line).Value(l));
						if (p == IntType(0))
							p = string_pos(/*" - "*/ STR(3794), idArr(tbx, line).Value(l));
						if (p == IntType(0))
				...
				for (l = IntType(0); l < idVar(tbx, lines); l++)
				{
					if (string_width(idArr(tbx, line).Value(l)) > w)
					{
						...
						for (p = string_length(idArr(tbx, line).Value(l)); p > IntType(1); p--)
						{
							...
							if ((a == /*" "*/ STR(20) || a == /*"-"*/ STR(1089)) && string_width(string_copy(idArr(tbx, line).Value(l), IntType(1), (IntType)(p - IntType(1)))) < w)
								break;
						}
						if (p == IntType(1))
						{
				...
				if ((gmlGlobal::mouse_x >= xx || sVar(window_busy) == string(tbx)) && (gmlGlobal::mouse_x < xx + w || sVar(window_busy) == string(tbx)) && (gmlGlobal::mouse_y >= yy + ly || (sVar(window_busy) == string(tbx) && ly == IntType(0))) && (gmlGlobal::mouse_y < yy + ly + hh || (sVar(window_busy) == string(tbx) && (ly + lineheight > h || l == idVar(tbx, lines) - IntType(1)))))
				{
					if (sBool(mouse_left))
					{
						if (idReal(tbx, select_on_focus) > 0 && sVar(textbox_lastfocus) != tbx)
						{
							...
							sVar(window_busy) = string(tbx) + /*"click"*/ STR(1062);
						}
						else
						{
								...
								if (sVar(textbox_select_clickline) == sVar(textbox_select_mouseline) && sVar(textbox_select_clickpos) == sVar(textbox_select_mousepos))
								{
									if (current_time() - sInt(textbox_click) < IntType(500))
									{
										...
										while (true)
										{
											...
											if (c == /*" "*/ STR(20) || c == /*", "*/ STR(1029) || c == /*"("*/ STR(1084) || c == /*")"*/ STR(1088) || c == /*"["*/ STR(977) || c == /*"]"*/ STR(1213) || c == /*"+"*/ STR(1090))
												break;
											sVar(textbox_select_startpos)--;
											if (sVar(textbox_select_startpos) <= IntType(0))
											{
