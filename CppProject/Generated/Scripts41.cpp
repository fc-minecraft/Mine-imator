...
namespace CppProject
{
	...
	void popup_about_draw(ScopeAny self)
	{
		RealType dx, dy;
		VarType dw, dh;
		dx = floor(sVar(content_x) + 106.0);
		dy = floor(sVar(content_y) + 10.0);
		draw_label(self, { text_get({ /*"aboutversion"*/ STR(1937), mineimator_version_full }), dx, dy });
		dy += IntType(18);
		draw_label(self, { text_get({ /*"aboutreleasedate"*/ STR(1938), mineimator_version_date }), dx, dy });
		dy += IntType(26);
		RealType mctextx = dx + string_width(text_get({ /*"aboutminecraftpre"*/ STR(1939) }));
		draw_label(self, { text_get({ /*"aboutminecraftpre"*/ STR(1939) }) + text_get({ /*"aboutminecraft"*/ STR(1940) }), dx, dy });
		dy += IntType(26);
		RealType creditsy = dy;
		RealType textx = dx;
		draw_label(self, { text_get({ /*"aboutcreatedby"*/ STR(1941) }), dx, dy });
		dy += IntType(20);
		draw_label(self, { /*"David Andrei"*/ STR(1942), dx, dy, sInt(font_label) });
		dy += IntType(34);
		draw_label(self, { text_get({ /*"aboutdevelopment"*/ STR(1943) }), dx, dy });
		dy += IntType(20);
		draw_label(self, { /*"Nimi"*/ STR(1944), dx, dy, sInt(font_label) });
		dy += IntType(34);
		draw_image({ sVar(spr_about), IntType(0), sVar(content_x) + IntType(18), sVar(content_y) + IntType(35) });
	}
	
	void popup_advanced_draw(ScopeAny self)
	{
		draw_set_font(sInt(font_value));
		StringType text = string_limit_ext(text_get({ /*"advancedinfo"*/ STR(1945) }), (sVar(dw) - IntType(40)) + IntType(8), no_limit);
		...
		IntType buttonx = string_width(text_get({ /*"advancedenable"*/ STR(1946) })) + button_padding;
		tab_control_button_label(self);
		if (draw_button_label(self, /*"advancedenable"*/ STR(1946), sVar(dx) + sVar(dw) - buttonx, sVar(dy)))
		{
			action_setting_program_mode(self, true);
			popup_close(self);
		}
		buttonx += IntType(12) + (string_width(text_get({ /*"advancednotnow"*/ STR(1947) })) + button_padding);
		if (draw_button_label(self, /*"advancednotnow"*/ STR(1947), sVar(dx) + sVar(dw) - buttonx, sVar(dy), null_, null_, e_button_SECONDARY))
			popup_close(self);
		tab_next(self);
	}
	...
	VarType popup_armor_editor_draw_piece_type(ScopeAny self, StringType piece, IntType pieceid, RealType capwid)
	{
		...
		for (IntType i = IntType(0); i < statelen; i += IntType(2))
		{
			...
			VarType model = DsMap(ObjType(obj_minecraft_assets, global::mc_assets)->model_name_map).Value(/*"armor"*/ STR(753));
			if (state != piece)
				continue;
			...
			if (type == /*"leather"*/ STR(1771))
			{
				w -= (28.0 + 8.0);
				if (draw_button_color(self, /*"armoreditordye"*/ STR(1948) + piece, sVar(dx) + sVar(dw) - IntType(24), sVar(dy), IntType(24), idArr(idVar(sInt(popup), armor_edit), armor_array).Value(pieceid + IntType(1)), minecraft_get_color(/*"other:leather"*/ STR(1949)), false, ID_action_armor_editor))
				{
	...
	void popup_armor_editor_draw_piece(ScopeAny self, StringType piece, IntType pieceid, RealType capwid)
	{
		if (sStr(settings_menu_name) != /*""*/ STR(0))
			sVar(settings_menu_busy_prev) = /*"popup"*/ STR(1593) + idVar(sInt(popup), name);
		if (sVar(context_menu_name) != /*""*/ STR(0))
			sVar(context_menu_busy_prev) = /*"popup"*/ STR(1593) + idVar(sInt(popup), name);
		if (ds_list_size(sInt(menu_list)) > IntType(0))
			ObjType(obj_menu, DsList(sInt(menu_list))[IntType(0)])->menu_busy_prev = /*"popup"*/ STR(1593) + idVar(sInt(popup), name);
		...
		tab_control(self, IntType(24));
		draw_button_menu(self, { /*"armoreditorpattern"*/ STR(1492) + piece, e_menu_LIST, sVar(dx), sVar(dy), sVar(dw), IntType(24), idArr(idVar(sInt(popup), armor_edit), armor_array).Value(pieceid + IntType(2)), text_get({ /*"armoreditorpattern"*/ STR(1492) + idArr(idVar(sInt(popup), armor_edit), armor_array).Value(pieceid + IntType(2)) }), ID_action_armor_editor, piecetype == /*"none"*/ STR(880), null_, null_, /*""*/ STR(0), c_white, IntType(1), capwid });
		tab_next(self);
		ObjType(obj_popup, sInt(popup_armor_editor))->piece_data_id = IntType(3);
		tab_control(self, IntType(24));
		draw_button_menu(self, { /*"armoreditormaterial"*/ STR(1497) + piece, e_menu_LIST, sVar(dx), sVar(dy), sVar(dw), IntType(24), idArr(idVar(sInt(popup), armor_edit), armor_array).Value(pieceid + IntType(3)), text_get({ /*"armoreditormaterial"*/ STR(1497) + idArr(idVar(sInt(popup), armor_edit), armor_array).Value(pieceid + IntType(3)) }), ID_action_armor_editor, (piecetype == /*"none"*/ STR(880) || idArr(idVar(sInt(popup), armor_edit), armor_array).Value(pieceid + IntType(2)) == /*"none"*/ STR(880)), null_, null_, /*""*/ STR(0), c_white, IntType(1), capwid });
		tab_next(self);
	}
	...
	void popup_armor_editor_draw(ScopeAny self)
	{
		...
		popup_armor_editor_draw_piece(self, /*"helmet"*/ STR(1950), IntType(0), capwid);
		draw_divide(sVar(dx), sVar(dy), sVar(dw));
		sVar(dy) += IntType(8);
		popup_armor_editor_draw_piece(self, /*"chestplate"*/ STR(1951), IntType(4), capwid);
		draw_divide(sVar(dx), sVar(dy), sVar(dw));
		sVar(dy) += IntType(8);
		popup_armor_editor_draw_piece(self, /*"leggings"*/ STR(1952), IntType(8), capwid);
		draw_divide(sVar(dx), sVar(dy), sVar(dw));
		sVar(dy) += IntType(8);
		popup_armor_editor_draw_piece(self, /*"boots"*/ STR(1953), IntType(12), capwid);
	}
	...
	void popup_close(ScopeAny self)
	{
		...
		sStr(popup_ani_type) = /*"hide"*/ STR(722);
	...
	RealType popup_draw(ScopeAny self)
	{
		...
		if (sStr(popup_ani_type) == /*"show"*/ STR(723))
		{
			...
			if (sStr(popup_ani_type) == /*"hide"*/ STR(722))
			{
		...
		if (!(sInt(popup) > 0) || (sStr(popup_ani_type) == /*"hide"*/ STR(722) && sInt(popup_switch_to) == null_))
		{
			sVar(popup_block_ani) = max({ IntType(0), test_reduced_motion(IntType(0), (sVar(popup_block_ani) - 0.075 * global::delta)) });
			sStr(popup_block_ani_ease) = /*"easeincirc"*/ STR(570);
			sBool(popup_mouseon) = false;
			if (!(sInt(popup) > 0))
				return IntType(0);
		}
		else
			if (idVar(sInt(popup), block) > 0 || (sInt(popup_switch_to) != null_ && idVar(sInt(popup_switch_to), block) > 0 && sStr(popup_ani_type) == /*"hide"*/ STR(722)))
			{
		...
		draw_set_alpha(ease(/*"easeoutcirc"*/ STR(571), sVar(popup_ani)));
		if (sVar(window_busy) == /*"popupmove"*/ STR(1959))
			draw_set_alpha(0.5);
		if (sVar(window_busy) == /*"popup"*/ STR(1593) + idVar(sInt(popup), name))
			sVar(window_busy) = /*""*/ STR(0);
		...
		if (idVar(sInt(popup), close_button) > 0)
		{
			...
			if (draw_button_icon(self, { idVar(sInt(popup), name) + /*"close"*/ STR(1083), closex, closey, IntType(24), IntType(24), false, (revert > 0) ? icons_ARROW_LEFT : icons_CLOSE, null_, false }))
			{
				if (revert > 0)
	...
	void popup_exportimage_draw(ScopeAny self)
	{
		...
		draw_button_menu(self, { /*"exportimageimagesize"*/ STR(1430), e_menu_LIST, sVar(dx), sVar(dy), sVar(dw), IntType(24), sVar(project_video_template), sVar(text), ID_action_project_video_template });
		tab_next(self);
		if (sVar(project_video_template) == IntType(0))
		{
	...
	void popup_exportmovie_draw(ScopeAny self)
	{
		...
		if (sVar(project_video_template) == IntType(0))
			...
			text = text_get({ /*"projectvideosizetemplate"*/ STR(1433) + ObjType(obj_videotemplate, sVar(project_video_template))->name }) + /*" ("*/ STR(1188) + string(ObjType(obj_videotemplate, sVar(project_video_template))->width) + /*"x"*/ STR(822) + string(ObjType(obj_videotemplate, sVar(project_video_template))->height) + /*")"*/ STR(1088);
		
		tab_control_menu(self);
		draw_button_menu(self, { /*"exportmovievideosize"*/ STR(1429), e_menu_LIST, sVar(dx), sVar(dy), sVar(dw), IntType(24), sVar(project_video_template), text, ID_action_project_video_template });
		tab_next(self);
		if (sVar(project_video_template) == IntType(0))
		{
		...
		tab_control_menu(self);
		draw_button_menu(self, { /*"exportmovieformat"*/ STR(1435), e_menu_LIST, sVar(dx), sVar(dy), sVar(dw), IntType(24), idStr(sInt(popup), format), text_get({ /*"exportmovieformat"*/ STR(1435) + idStr(sInt(popup), format) }), ID_action_toolbar_exportmovie_format });
		tab_next(self);
		if (idStr(sInt(popup), format) != /*"png"*/ STR(64))
		{
			if (idInt(sInt(popup), video_quality) == IntType(0))
				text = text_get({ /*"exportmovievideoqualitycustom"*/ STR(1441) });
			else
				text = text_get({ /*"exportmovievideoquality"*/ STR(1440) + ObjType(obj_videoquality, idInt(sInt(popup), video_quality))->name });
			
			tab_control_menu(self);
			draw_button_menu(self, { /*"exportmovievideoquality"*/ STR(1440), e_menu_LIST, sVar(dx), sVar(dy), sVar(dw), IntType(24), idInt(sInt(popup), video_quality), text, ID_action_toolbar_exportmovie_video_quality });
			tab_next(self);
			if (idInt(sInt(popup), video_quality) == IntType(0))
			{
		...
		if (idVar(sInt(popup), frame_rate) == IntType(0))
			text = text_get({ /*"exportmovieframeratecustom"*/ STR(1446) });
		else
			text = string(idVar(sInt(popup), frame_rate));
		
		tab_control_menu(self);
		draw_button_menu(self, { /*"exportmovieframerate"*/ STR(1442), e_menu_LIST, sVar(dx), sVar(dy), sVar(dw), IntType(24), idVar(sInt(popup), frame_rate), text, ID_action_toolbar_exportmovie_frame_rate });
		tab_next(self);
		if (idVar(sInt(popup), frame_rate) == IntType(0))
		{
			tab_control_dragger(self);
	...
	{
		draw_label(self, { text_get({ /*"importimagetype"*/ STR(1983) }) + /*":"*/ STR(760), sVar(dx), sVar(dy) + IntType(14), fa_left, fa_bottom, global::c_text_secondary, global::a_text_secondary, sInt(font_label) });
		sVar(dy) += IntType(28);
		tab_control_checkbox(self);
	...
	void popup_newproject_draw(ScopeAny self)
	{
		...
		if (draw_textfield(self, /*"newprojectname"*/ STR(2000), sVar(dx), sVar(dy), sVar(dw), IntType(24), idInt(sInt(popup), tbx_name), null_, idVar(sInt(popup), folder), /*"top"*/ STR(196)) || issue)
		{
		...
		tab_next(self);
		tab_control_textfield(self, true);
		if (draw_textfield(self, /*"newprojectauthor"*/ STR(2001), sVar(dx), sVar(dy), sVar(dw), IntType(24), idInt(sInt(popup), tbx_author), null_, /*""*/ STR(0), /*"top"*/ STR(196)))
		{
			idVar(sInt(popup), author) = ObjType(obj_textbox, idInt(sInt(popup), tbx_author))->text;
		}
		tab_next(self);
		tab_control_textfield(self, true, IntType(76));
		if (draw_textfield(self, /*"newprojectdescription"*/ STR(2002), sVar(dx), sVar(dy), sVar(dw), IntType(76), idInt(sInt(popup), tbx_description), null_, /*""*/ STR(0), /*"top"*/ STR(196)))
		{
	...
	RealType popup_pattern_editor_draw_layer(ScopeAny self, VarType xx, VarType yy, RealType width, RealType height, VarType index, RealType base)
	{
		...
		layercolor = ((base > 0) ? minecraft_get_color(/*"dye:black"*/ STR(2012)) : DsList(ObjType(obj_popup, sInt(popup_pattern_editor))->pattern_color_list_edit).Value(index));
		if (ObjType(obj_popup, sInt(popup_pattern_editor))->pattern_edit_preview != null_)
		...
		if (base > 0)
		{
			...
			name = text_get({ /*"patterneditorbase"*/ STR(2013), text_get({ /*"swatchdye"*/ STR(2014) + colorname }) });
		}
		else
		{
			...
			name = text_get({ /*"patterneditorlayer"*/ STR(2015), text_get({ /*"swatchdye"*/ STR(2014) + colorname }), text_get({ /*"patterneditorpatterns"*/ STR(2016) + DsList(ObjType(obj_popup, sInt(popup_pattern_editor))->pattern_list_edit).Value(index) }) });
		}
		...
		if (!(base > 0))
		{
			...
			if (draghover)
			{
				...
				if (sReal(mouse_move) > IntType(0) && !dragging)
				{
					...
					sVar(window_busy) = /*"patterneditordraglayer"*/ STR(2017);
				}
			}
			...
			if (mouseon && !dragging)
			{
				if (draw_button_icon(self, { /*"patterneditorremovelayer"*/ STR(2018) + string(index), actionx - IntType(24), yy + height / 2.0 - IntType(14), IntType(24), IntType(24), false, icons_DELETE }))
		...
		if (mouseon && !dragging)
		{
			...
			if (sStr(settings_menu_name) == editname && sStr(settings_menu_ani_type) != /*"hide"*/ STR(722))
