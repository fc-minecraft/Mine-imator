...
namespace CppProject
{
	...
	void tl_keyframes_paste(ScopeAny self, VarType pos)
	{
		...
		for (IntType k = IntType(0); k < sInt(copy_kf_amount); k++)
		{
			...
			{
				pastemode = /*"fixed"*/ STR(3803);
				break;
			}
			tllast = sArr(copy_kf_tl_save_id).Value(k);
		}
		if (pastemode == /*"fixed"*/ STR(3803))
		{
		...
		for (IntType k = IntType(0); k < sInt(copy_kf_amount); k++)
		{
			...
			if (tladd == null_)
				continue;
			if (pastemode != /*"free"*/ STR(3802) && idInt(tladd, part_list) != null_ && sArr(copy_kf_tl_model_part_name).Value(k) != /*""*/ STR(0))
			{
				VarType part;
	...
	void tl_update_display_name(ScopeAny self)
	{
		if (sVar(name) == /*""*/ STR(0))
		{
			sVar(display_name) = text_get({ /*"type"*/ STR(777) + DsList(global::tl_type_name_list).Value(sVar(type)) });
			if (sVar(part_of) != null_)
			{
				if (sVar(type) == e_tl_type_BODYPART)
				{
					if (sVar(model_part) != null_)
						sVar(display_name) = minecraft_asset_get_name(/*"modelpart"*/ STR(750), idVar(sVar(model_part), name));
					else
						sVar(display_name) = text_get({ /*"timelineunusedbodypart"*/ STR(3804) });
