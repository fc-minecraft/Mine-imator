...
namespace CppProject
{
	...
	RealType time_get_rotation(VarType str)
	{
		...
		colons = string_count(/*":"*/ STR(760), str);
		days = IntType(0);
		hours = IntType(0);
		minutes = IntType(0);
		if (colons > IntType(2))
		...
		if (colons == IntType(1))
		{
			colonpos = string_pos(/*":"*/ STR(760), str);
	...
	void tl_event_create(Scope<obj_timeline> self)
	{
		...
		self->temp = null_;
		self->text = text_get({ /*"timelineeditortextsample"*/ STR(3795) });
		...
		self->blend_mode = /*"normal"*/ STR(603);
		...
		self->text_res = null_;
		self->text_3d = false;
		self->text_halign_prev = /*"center"*/ STR(1422);
		self->text_valign_prev = /*"center"*/ STR(1422);
	...
	void tl_filter_draw(ScopeAny self)
	{
		...
		RealType colorwid;
		VarType px;
		switchwid = text_max_width({ /*"timelinehideghosts"*/ STR(3796) }) + IntType(16) + IntType(24);
		colorwid = max({ text_max_width({ /*"timelinefiltertags"*/ STR(3797) }), (IntType(24) * IntType(9)) - IntType(4) });
		draw_label(self, { text_get({ /*"timelinefiltertags"*/ STR(3797) }), sVar(dx), sVar(dy) + IntType(9), fa_left, fa_middle, global::c_text_secondary, global::a_text_secondary, sInt(font_label) });
		sVar(dy) += IntType(20);
		...
		for (IntType i = IntType(0); i <= IntType(8); i++)
		{
			...
			microani_set(/*"hidecolortag"*/ STR(3798) + string(i), null_, false, false, sVar(timeline_hide_color_tag).Value(i));
		...
		draw_switch(self, /*"timelinehideghosts"*/ STR(3796), sVar(dx), sVar(dy), sVar(setting_timeline_hide_ghosts), ID_action_setting_timeline_hide_ghosts, /*"timelinehideghoststip"*/ STR(3799));
		tab_next(self);
	...
	BoolType tl_get_visible(ScopeAny self)
	{
		...
		if (global::render_active == /*"image"*/ STR(94))
		{
			...
			if (global::render_active == /*"movie"*/ STR(1109))
			{
	...
	void tl_interval_settings_draw(ScopeAny self)
	{
		...
		draw_dragger(self, /*"timelineintervalssize"*/ STR(3800), sVar(dx), sVar(dy), dragger_width, sVar(timeline_interval_size), .1, IntType(1), no_limit, sVar(project_tempo), IntType(1), idInt(sVar(timeline), tbx_interval_size), ID_action_tl_interval_size);
		tab_next(self);
		tab_control_dragger(self);
		draw_dragger(self, /*"timelineintervalsoffset"*/ STR(3801), sVar(dx), sVar(dy), dragger_width, sVar(timeline_interval_offset), .1, -no_limit, no_limit, IntType(0), IntType(1), idInt(sVar(timeline), tbx_interval_offset), ID_action_tl_interval_offset);
		tab_next(self);
		sVar(settings_menu_w) = (text_max_width({ /*"timelineintervalssize"*/ STR(3800), /*"timelineintervalsoffset"*/ STR(3801) }) + IntType(16) + dragger_width) + IntType(24);
	}
