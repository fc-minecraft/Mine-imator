...
namespace CppProject
{
	void minecraft_assets_load_biomes(IntType list, VarType map)
	{
		for (IntType b = IntType(0); b < ds_list_size((IntType)(map)); b++)
		{
			...
			withOne (obj_biome, (new obj_biome)->id, noone)
			{
				...
				self->group = value_get_real(DsMap(biome).Value(/*"group"*/ STR(1728)), false);
	...
	{
		ArrType letterlist = ArrType::From({ /*"a"*/ STR(1736), /*"b"*/ STR(1737), /*"c"*/ STR(1738), /*"d"*/ STR(1739), /*"e"*/ STR(1215), /*"f"*/ STR(1216), /*"g"*/ STR(1740), /*"h"*/ STR(1741), /*"i"*/ STR(1742), /*"j"*/ STR(1743), /*"k"*/ STR(1744), /*"l"*/ STR(1745), /*"m"*/ STR(1746), /*"n"*/ STR(1217), /*"o"*/ STR(1747), /*"p"*/ STR(1748), /*"q"*/ STR(1749), /*"r"*/ STR(1750), /*"s"*/ STR(1751), /*"t"*/ STR(1218), /*"u"*/ STR(1219), /*"v"*/ STR(1752), /*"w"*/ STR(1753), /*"x"*/ STR(822), /*"y"*/ STR(805), /*"z"*/ STR(823) });
		for (IntType i = IntType(0); i < ds_list_size((IntType)(particlelist)); i++)
		{
			...
			if (!is_undefined(DsMap(tempmap).Value(/*"frames"*/ STR(1754))))
			{
				ObjType(obj_particle_template, ptemp)->frames = DsMap(tempmap).Value(/*"frames"*/ STR(1754));
			...
			ObjType(obj_particle_template, ptemp)->letter_suffix = value_get_real(DsMap(tempmap).Value(/*"letter_suffix"*/ STR(1755)), false);
	...
	BoolType minecraft_assets_load_startup(ScopeAny self)
	{
		...
		withOne (obj_resource, global::mc_res, self->id)
		{
			...
			self->display_name = /*"Minecraft"*/ STR(1756);
		...
		{
			log({ /*"Could not load "*/ STR(1757) + string(global::_app->setting_minecraft_assets_version) + /*" assets. Resetting to default"*/ STR(1758), minecraft_version });
		...
		if (file_exists_lib(splash_directory + /*"splashes.json"*/ STR(1759)))
		{
			IntType map;
			VarType splashlist, splash;
			StringType splashfile;
			map = json_load({ splash_directory + /*"splashes.json"*/ STR(1759) });
			splashlist = DsMap(map).Value(/*"splashes"*/ STR(1760));
			...
			if (file_exists_lib(splashfile))
			{
				...
				global::load_assets_credits = DsMap(splash).Value(/*"credits"*/ STR(1761));
			}
		}
		return true;
	}
	...
	BoolType minecraft_assets_load_startup_version()
	{
		...
		log({ /*"Loading Minecraft assets version"*/ STR(1762), version });
		...
		{
			log({ /*"Could not find Minecraft assets file"*/ STR(1763), global::load_assets_file });
			return false;
		}
		if (!file_exists_lib(global::load_assets_zip_file))
		{
			log({ /*"Could not find Minecraft assets archive"*/ STR(1764), global::load_assets_zip_file });
			return false;
		}
		...
		{
			log({ /*"Could not parse JSON"*/ STR(1765), global::load_assets_file });
		...
		{
			log({ /*"Unsupported archive, format"*/ STR(1766), format });
			return false;
		}
		VarType patch = DsMap(global::load_assets_map).Value(/*"patch"*/ STR(1767));
		if (!is_real(patch))
			patch = IntType(1);
		global::load_assets_startup_dir = mc_file_directory + version + (patch > IntType(1) ? /*"_"*/ STR(1110) + string(patch) : /*""*/ STR(0)) + /*"/"*/ STR(19);
	...
	BoolType minecraft_assets_reload()
	{
		...
		log({ /*"Reloading models"*/ STR(1768) });
		IntType typemap, map;
		typemap = ds_int_map_create();
		map = json_load({ fname, typemap });
		if (!ds_map_valid(map))
		{
			log({ /*"Could not parse JSON"*/ STR(1765), fname });
		...
		withOne (obj_minecraft_assets, global::mc_assets, noone)
		{
			...
			VarType characterslist = DsMap(map).Value(/*"characters"*/ STR(1703));
			...
			VarType specialblockslist = DsMap(map).Value(/*"special_blocks"*/ STR(1707));
	...
	ArrType minecraft_update_armor_generate(ArrType data, VarType res)
	{
		...
		for (IntType i = IntType(0); i < IntType(4); i++)
		{
			...
			basetex = /*"models/armor/"*/ STR(1769) + basemat + /*"_layer_"*/ STR(1770) + layernum;
			...
			surface_set_target(armorsurf);
		{
			...
			if (basemat == /*"leather"*/ STR(1771))
			{
				draw_texture({ basesprite, IntType(0), IntType(0), IntType(1), IntType(1), dye, IntType(1) });
				draw_texture({ DsMap(idInt(res, model_texture_map)).Value(basetex + /*"_overlay"*/ STR(1772)), IntType(0), IntType(0) });
			}
			else
			...
			{
				VarType palette = DsMap(idInt(res, model_texture_map)).Value(/*"trims/color_palettes/"*/ STR(1773) + trimmat + (trimmat == basemat ? /*"_darker"*/ STR(1774) : /*""*/ STR(0)));
				surface_set_target(armorsurf);
			{
				...
				withOne (Object, global::render_shader_obj, noone)
				{
					...
					shader_palette_set(self, palette, DsMap(idInt(res, model_texture_map)).Value(/*"trims/color_palettes/trim_palette"*/ STR(1775)));
				}
				
				if (i == IntType(2))
					draw_sprite((IntType)(DsMap(idInt(res, model_texture_map)).Value(/*"trims/models/armor/"*/ STR(1776) + trimpat + /*"_leggings"*/ STR(1777))), IntType(0), IntType(0), IntType(0));
				else
					draw_sprite((IntType)(DsMap(idInt(res, model_texture_map)).Value(/*"trims/models/armor/"*/ STR(1776) + trimpat)), IntType(0), IntType(0), IntType(0));
	...
	IntType minecraft_update_pattern_generate(VarType type, VarType color, VarType patternlist, VarType colorlist, VarType res)
	{
		...
		patternbase = (type == /*"banner"*/ STR(788) ? /*"entity/banner_base"*/ STR(1778) : /*"entity/shield_base"*/ STR(1779));
		patterndir = (type == /*"banner"*/ STR(788) ? /*"entity/banner/"*/ STR(1780) : /*"entity/shield/"*/ STR(1781));
		skinratio = IntType(1);
		maskarray = ArrType();
	...
	{
		log({ /*"Missing file"*/ STR(1782), fn });
		show_message(/*"The file "*/ STR(1783) + fn + /*" could not be found. If you are running from an archive, extract all the files and try again. If problems persist, re-install the program."*/ STR(1784));
		return false;
	}
	...
	IntType model_file_load(StringType fname, IntType res)
	{
		...
		{
			log({ /*"Missing parameter \"texture\""*/ STR(1785) });
			return null_;
		}
		if (!ds_list_valid(DsMap(map).Value(/*"texture_size"*/ STR(1786))))
		{
			log({ /*"Missing array \"texture_size\""*/ STR(1787) });
			return null_;
		}
		if (!ds_list_valid(DsMap(map).Value(/*"parts"*/ STR(1788))))
		{
			log({ /*"Missing array \"parts\""*/ STR(1789) });
			return null_;
		}
		withOne (obj_model_file, (new obj_model_file)->id, noone)
		{
			...
			if (res == null_ && dev_mode_debug_names && !text_exists(/*"model"*/ STR(8) + self->name))
				log({ /*"model/"*/ STR(1790) + self->name + dev_mode_name_translation_message });
			self->description = value_get_string(DsMap(map).Value(/*"description"*/ STR(1791)), /*""*/ STR(0));
			...
			if (res != null_)
			{
				self->texture_material_name = value_get_string(DsMap(map).Value(/*"texture_material"*/ STR(1792)), /*""*/ STR(0));
				if (self->texture_material_name != /*""*/ STR(0))
					model_file_load_texture_material(self->texture_material_name, res);
				self->texture_normal_name = value_get_string(DsMap(map).Value(/*"texture_normal"*/ STR(1793)), /*""*/ STR(0));
			...
			self->texture_size = value_get_point2D(DsMap(map).Value(/*"texture_size"*/ STR(1786)));
			...
			self->player_skin = value_get_real(DsMap(map).Value(/*"player_skin"*/ STR(1794)), false);
			self->scale = value_get_point3D(DsMap(map).Value(/*"scale"*/ STR(861)), vec3(IntType(1), IntType(1), IntType(1)));
			self->model_color = value_get_string(DsMap(map).Value(/*"model_color"*/ STR(1795)), /*"none"*/ STR(880));
			...
			self->floor_box_uvs = value_get_real(DsMap(map).Value(/*"floor_box_uvs"*/ STR(1796)), false);
			VarType partlist = DsMap(map).Value(/*"parts"*/ STR(1788));
	...
	{
		if (!is_undefined(DsMap(map).Value(/*"visible"*/ STR(1797))) && !(DsMap(map).Value(/*"visible"*/ STR(1797)) > 0))
			return IntType(0);
		if (!is_string(DsMap(map).Value(/*"name"*/ STR(791))))
		{
			log({ /*"Missing parameter \"name\""*/ STR(792) });
		...
		{
			log({ /*"Missing array \"position\""*/ STR(1798) });
			return null_;
		}
		for (IntType i = IntType(0); i < ds_list_size(ObjType(obj_model_file, root)->file_part_list); i++)
		{
			...
			{
				log({ /*"Duplicate part name found"*/ STR(1799), DsMap(map).Value(/*"name"*/ STR(791)) });
				return null_;
			}
		}
		withOne (obj_model_part, (new obj_model_part)->id, self->id)
		{
			...
			if (res == null_ && dev_mode_debug_names && !text_exists(/*"modelpart"*/ STR(750) + self->name))
				log({ /*"model/part/"*/ STR(1800) + self->name + dev_mode_name_translation_message });
			...
			self->color_inherit = value_get_real(DsMap(map).Value(/*"color_inherit"*/ STR(1801)), true);
			self->color_blend = value_get_color(DsMap(map).Value(/*"color_blend"*/ STR(1802)), c_white);
			self->color_alpha = value_get_real(DsMap(map).Value(/*"color_alpha"*/ STR(1803)), IntType(1));
			if (is_real(/*"color_emissive"*/ STR(1804)))
				self->color_emissive = value_get_real(DsMap(map).Value(/*"color_emissive"*/ STR(1804)), IntType(0));
			else
				self->color_emissive = value_get_real(DsMap(map).Value(/*"color_brightness"*/ STR(1805)), IntType(0));
			
			self->color_mix = value_get_color(DsMap(map).Value(/*"color_mix"*/ STR(1806)), c_black);
			self->color_mix_percent = value_get_real(DsMap(map).Value(/*"color_mix_percent"*/ STR(1807)), IntType(0));
			self->part_mixing_shapes = false;
			...
			self->show_position = value_get_real(DsMap(map).Value(/*"show_position"*/ STR(1808)), false);
			...
			{
				if (is_bool(DsMap(map).Value(/*"lock_bend"*/ STR(1809))) || is_real(DsMap(map).Value(/*"lock_bend"*/ STR(1809))))
					self->lock_bend = DsMap(map).Value(/*"lock_bend"*/ STR(1809));
				if (self->lock_bend > 0)
				{
			...
			self->backfaces = value_get_real(DsMap(map).Value(/*"backfaces"*/ STR(1810)), false);
			if (!is_undefined(DsMap(map).Value(/*"bend"*/ STR(1811))))
			{
				VarType bendmap = DsMap(map).Value(/*"bend"*/ STR(1811));
				self->bend_inherit = value_get_real(DsMap(bendmap).Value(/*"inherit_bend"*/ STR(1812)), false);
				if (!is_real(DsMap(bendmap).Value(/*"offset"*/ STR(1813))))
				{
					log({ /*"Missing parameter \"offset\""*/ STR(1814) });
					return null_;
				}
				self->bend_offset = DsMap(bendmap).Value(/*"offset"*/ STR(1813));
				self->bend_end_offset = value_get_real(DsMap(bendmap).Value(/*"end_offset"*/ STR(1815)), IntType(0));
				self->bend_size = value_get_real(DsMap(bendmap).Value(/*"size"*/ STR(992)), null_);
				if (!is_string(DsMap(bendmap).Value(/*"part"*/ STR(875))))
				{
					log({ /*"Missing parameter \"part\""*/ STR(1816) });
					return null_;
				}
				switch (StringType(DsMap(bendmap).Value(/*"part"*/ STR(875))).id)
				{
					...
					default:
						log({ /*"Invalid parameter \"part\""*/ STR(1818) });
						return null_;
				}
				...
				if (is_string(DsMap(bendmap).Value(/*"axis"*/ STR(804))))
				{
					switch (StringType(DsMap(bendmap).Value(/*"axis"*/ STR(804))).id)
					{
						...
						default:
							log({ /*"Invalid parameter \"axis\""*/ STR(1819) });
							return null_;
					}
					...
					{
						log({ /*"Missing parameter \"axis\""*/ STR(1820) });
						return null_;
					}
				
				
				self->bend_direction_min = vec3(-IntType(180));
				if (is_real(DsMap(bendmap).Value(/*"direction_min"*/ STR(1821))) && array_length(VarType::CreateRef(axis)) == IntType(1))
				{
					self->bend_direction_min.Real(axis.Value(IntType(0))) = DsMap(bendmap).Value(/*"direction_min"*/ STR(1821));
				}
				else
					if (ds_list_valid(DsMap(bendmap).Value(/*"direction_min"*/ STR(1821))))
					{
						for (IntType i = IntType(0); i < ds_list_size((IntType)(DsMap(bendmap).Value(/*"direction_min"*/ STR(1821)))); i++)
							self->bend_direction_min.Real(axis.Value(i)) = ds_list_find_value((IntType)(DsMap(bendmap).Value(/*"direction_min"*/ STR(1821))), i);
					}
				
				self->bend_direction_max = vec3(IntType(180));
				if (is_real(DsMap(bendmap).Value(/*"direction_max"*/ STR(1822))) && array_length(VarType::CreateRef(axis)) == IntType(1))
				{
					self->bend_direction_max.Real(axis.Value(IntType(0))) = DsMap(bendmap).Value(/*"direction_max"*/ STR(1822));
				}
				else
					if (ds_list_valid(DsMap(bendmap).Value(/*"direction_max"*/ STR(1822))))
					{
						for (IntType i = IntType(0); i < ds_list_size((IntType)(DsMap(bendmap).Value(/*"direction_max"*/ STR(1822)))); i++)
							self->bend_direction_max.Real(axis.Value(i)) = ds_list_find_value((IntType)(DsMap(bendmap).Value(/*"direction_max"*/ STR(1822))), i);
					}
				...
				if (is_string(DsMap(bendmap).Value(/*"direction"*/ STR(902))))
				{
					switch (StringType(DsMap(bendmap).Value(/*"direction"*/ STR(902))).id)
					{
						...
						default:
							log({ /*"Invalid parameter \"direction\""*/ STR(1826) });
							return null_;
					}
				...
				if ((is_real(DsMap(bendmap).Value(/*"invert"*/ STR(1827))) || is_bool(DsMap(bendmap).Value(/*"invert"*/ STR(1827)))) && array_length(VarType::CreateRef(axis)) == IntType(1))
				{
					self->bend_invert = vec3(DsMap(bendmap).Value(/*"invert"*/ STR(1827)));
				}
				else
					if (ds_list_valid(DsMap(bendmap).Value(/*"invert"*/ STR(1827))))
					{
						for (IntType i = IntType(0); i < ds_list_size((IntType)(DsMap(bendmap).Value(/*"invert"*/ STR(1827)))); i++)
							self->bend_invert.Real(axis.Value(i)) = ds_list_find_value((IntType)(DsMap(bendmap).Value(/*"invert"*/ STR(1827))), i);
					}
					else
			...
			self->has_3d_plane = false;
			VarType shapelist = DsMap(map).Value(/*"shapes"*/ STR(1828));
