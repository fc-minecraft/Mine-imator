...
namespace CppProject
{
		...
		switch ((IntType)vid)
		{
			...
			case e_value_TRANSITION:
			{
				switch ((IntType)val)
				{
					...
					case IntType(1):
					{
						return /*"easeinquad"*/ STR(552);
					}
					case IntType(2):
					{
						return /*"easeoutquad"*/ STR(553);
					}
					case IntType(3):
					{
						return /*"easeinoutquad"*/ STR(554);
					}
					case IntType(4):
					{
						return /*"instant"*/ STR(550);
					}
				}
	...
	void project_load_legacy_particles(ScopeAny self)
	{
		...
		ArrType boxtype = ArrType::From({ /*"none"*/ STR(880), /*"ground"*/ STR(142), /*"spawn"*/ STR(1333), /*"custom"*/ STR(602) });
	...
	void project_load_legacy_resource()
	{
		withOne (obj_resource, (new obj_resource)->id, noone)
		{
			...
			if (typename_ == /*"item"*/ STR(523))
				typename_ = /*"itemsheet"*/ STR(541);
			if (typename_ == /*"block"*/ STR(4) || typename_ == /*"blocksheet"*/ STR(543))
				typename_ = /*"legacyblocksheet"*/ STR(542);
			if (typename_ == /*"particles"*/ STR(525))
				typename_ = /*"particlesheet"*/ STR(545);
			if (typename_ == /*"schematic"*/ STR(2138))
				typename_ = /*"scenery"*/ STR(522);
	...
	BoolType project_load_legacy_start(VarType argument0)
	{
		...
		if (global::load_format > e_project_FORMAT_CB_103)
		{
			...
			{
				log({ /*"Too old legacy project, format"*/ STR(2141), global::load_format });
				error(/*"errorfilecorrupted"*/ STR(2140));
				buffer_delete(global::buffer_current);
				return false;
			}
	...
	void project_load_legacy_template()
	{
		withOne (obj_template, (new obj_template)->id, noone)
		{
			...
			if (self->type == e_temp_type_CHARACTER || self->type == e_temp_type_SPECIAL_BLOCK || self->type == e_temp_type_BODYPART)
			{
				...
				{
					self->model_name = DsMap(modelmap).Value(/*"name"*/ STR(791));
					if (!is_undefined(DsMap(modelmap).Value(/*"state"*/ STR(999))))
						self->model_state = string_get_state_vars(DsMap(modelmap).Value(/*"state"*/ STR(999)));
					else
					...
					log({ /*"Could not convert model "*/ STR(2142), self->legacy_model_name });
			...
			if (self->type == e_temp_type_BODYPART)
			{
				...
				if (ds_list_valid(modelpartlist) && self->legacy_bodypart_id < ds_list_size((IntType)(modelpartlist)))
					...
					log({ /*"Could not convert model part of "*/ STR(2143), self->model_name, self->legacy_bodypart_id });
			...
			if (self->text_font == /*"root"*/ STR(2127))
		...
		withOne (obj_timeline, (new obj_timeline)->id, noone)
		{
			...
			if (self->type == e_temp_type_BODYPART)
			{
				...
				if (!is_undefined(modelpartlist) && self->legacy_bodypart_id < ds_list_size((IntType)(modelpartlist)))
				{
					...
					log({ /*"Could not find model part for"*/ STR(2144), ObjType(obj_template, findtemp)->model_name, ObjType(obj_template, findtemp)->legacy_model_name, self->legacy_bodypart_id });
	...
	void project_load_legacy_values(ScopeAny self, IntType tl)
	{
		...
		if (idArr(tl, value_type).Value(e_value_type_PARTICLES))
		{
			...
			if (sVar(value).Value(e_value_ATTRACTOR) == /*"root"*/ STR(2127))
		...
		if (idArr(tl, value_type).Value(e_value_type_MATERIAL_TEXTURE))
		{
			...
			if (sVar(value).Value(e_value_TEXTURE_OBJ) == /*"root"*/ STR(2127))
	...
	RealType project_load_markers(VarType list)
	{
		...
		for (IntType i = IntType(0); i < ds_list_size((IntType)(list)); i++)
		{
			...
			withOne (obj_marker, (new obj_marker)->id, noone)
			{
				...
				self->name = value_get_string(DsMap(markermap).Value(/*"name"*/ STR(791)), /*""*/ STR(0));
				self->color = value_get_real(DsMap(markermap).Value(/*"color"*/ STR(892)), IntType(0));
			}
	...
	{
		VarType templist = DsMap(map).Value(/*"templates"*/ STR(2136));
		...
		VarType tllist = DsMap(map).Value(/*"timelines"*/ STR(2137));
