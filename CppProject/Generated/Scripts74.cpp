...
namespace CppProject
{
	RealType window_draw_load_assets(ScopeAny self)
	{
		...
		if (global::load_assets_stage == /*"done"*/ STR(1711))
		{
			global::load_assets_stage = /*"exit"*/ STR(3910);
			return IntType(0);
		}
		else
			if (global::load_assets_stage == /*"exit"*/ STR(3910))
			{
		...
		draw_label(self, { /*"Mine-imator "*/ STR(3911) + string(mineimator_version), xoff + IntType(95), yoff + IntType(289), fa_middle, fa_bottom, global::c_text_secondary, global::a_text_secondary, sInt(font_heading) });
		...
		draw_label(self, { text_get({ /*"startuploadingassets"*/ STR(3912), global::_app->setting_minecraft_assets_version, floor(global::load_assets_progress * IntType(100)) }), xoff + IntType(95), yoff + IntType(437), fa_middle, fa_bottom, global::c_text_tertiary, global::a_text_tertiary, sInt(font_caption) });
		...
		if (global::load_assets_credits != /*""*/ STR(0))
			draw_label(self, { text_get({ /*"startupsplashauthor"*/ STR(3913), global::load_assets_credits }), xoff + IntType(95), yoff + IntType(289) + IntType(31), fa_middle, fa_top, global::c_text_tertiary, global::a_text_tertiary, sInt(font_caption) });
	...
	RealType window_draw_new_assets(ScopeAny self)
	{
		...
		draw_label(self, { text_get({ /*"newassetstitle"*/ STR(3914), sStr(new_assets_version) }), sVar(dx) + (RealType)sVar(dw) / 2.0, sVar(dy) + IntType(227), fa_center, fa_bottom, global::c_accent, IntType(1), sInt(font_heading) });
		draw_label(self, { text_get({ /*"newassetssubtitle"*/ STR(3915) }), sVar(dx) + (RealType)sVar(dw) / 2.0, sVar(dy) + IntType(244), fa_center, fa_bottom, global::c_text_main, global::a_text_main, sInt(font_value) });
		sVar(dy) += IntType(264);
		sVar(dx) += IntType(12);
		sVar(dw) -= 24.0;
		if (sStr(new_assets_stage) == /*"download"*/ STR(109))
		{
			...
			draw_loading_bar(self, sVar(dx), sVar(dy), sVar(dw), IntType(8), sReal(new_assets_download_progress), text_get({ /*"newassetsdownloading"*/ STR(3916) }));
			tab_next(self);
		}
		else
		{
			draw_set_font(sInt(font_button));
			IntType capwid = string_width(text_get({ /*"newassetsdownload"*/ STR(3917) })) + button_padding;
			if (draw_button_label(self, /*"newassetsdownload"*/ STR(3917), sVar(dx) + sVar(dw), (sVar(content_y) + sVar(content_height)) - IntType(44), null_, null_, e_button_PRIMARY, null_, fa_right))
			{
				if (sVar(new_assets_format) > minecraft_assets_format)
				{
					if (question(text_get({ /*"questionassetsnewer"*/ STR(3918) })))
					{
						open_url(self, link_download);
						game_end();
						return IntType(0);
					}
				}
			...
			if (draw_button_label(self, /*"newassetslater"*/ STR(3919), (sVar(dx) + sVar(dw)) - (capwid + IntType(8)), (sVar(content_y) + sVar(content_height)) - IntType(44), null_, null_, e_button_SECONDARY, null_, fa_right))
			{
			...
			sVar(dy_start) = sVar(dy);
			sVar(dy) += IntType(16);
			draw_label(self, { text_get({ /*"newassetschangelog"*/ STR(3920) }), sVar(dx), sVar(dy), fa_left, fa_bottom, global::c_text_tertiary, global::a_text_tertiary, sInt(font_subheading) });
			sVar(dy) += IntType(20);
			for (IntType i = IntType(0); i < array_length(VarType::CreateRef(sArr(new_assets_changes_lines))); i++)
			{
				...
				if (string_copy(text, IntType(1), IntType(2)) == /*"- "*/ STR(3921))
				{
				...
				{
					if (string_copy(text, IntType(1), IntType(2)) == /*"* "*/ STR(3922))
						text = /*"• "*/ STR(3923) + string_delete(text, IntType(1), IntType(2));
	...
	void window_draw_startup(ScopeAny self)
	{
		...
		StringType trial = ((global::trial_version > 0) ? /*" "*/ STR(20) + text_get({ /*"startuptrial"*/ STR(3924) }) : /*""*/ STR(0));
		draw_button_text(self, { text_get({ /*"startupversion"*/ STR(3925), mineimator_version_full + trial }), (sReal(window_width) / 2.0) + IntType(259), floor((headersize / 2.0) + ((RealType)sprite_get_height(ID_spr_logo) / 2.0)) + IntType(3), ID_popup_switch, sInt(popup_about) });
		...
		{
			draw_label(self, { text_get({ /*"recentnone"*/ STR(3926) }), sReal(window_width) / 2.0, sVar(dy), fa_center, fa_middle, global::c_accent, IntType(1), sInt(font_heading_big) });
			sVar(dy) += IntType(48);
		}
		draw_set_font(sInt(font_button));
		RealType newprojectwidth, browsewidth, centerx;
		newprojectwidth = string_width(text_get({ /*"startupnewproject"*/ STR(3927) })) + button_icon_padding;
		browsewidth = string_width(text_get({ /*"startupbrowse"*/ STR(3928) })) + button_icon_padding;
		...
		if (draw_button_label(self, /*"startupnewproject"*/ STR(3927), sVar(dx), sVar(dy), null_, icons_FILE))
		{
		...
		if (draw_button_label(self, /*"startupbrowse"*/ STR(3928), sVar(dx), sVar(dy), null_, icons_FOLDER, e_button_SECONDARY))
		{
		...
		if (sReal(recent_list_amount) > IntType(0))
		{
			if (draw_button_icon(self, { /*"startuprecentdisplay"*/ STR(3929), sVar(dx) - IntType(24) - IntType(8), sVar(dy) + IntType(4), IntType(24), IntType(24), false, sStr(recent_display_mode) == /*"grid"*/ STR(615) ? icons_VIEW_LIST : icons_VIEW_GRID, null_, false, sStr(recent_display_mode) == /*"grid"*/ STR(615) ? /*"tooltipviewlist"*/ STR(3930) : /*"tooltipviewgrid"*/ STR(3931) }))
			{
		...
		if (sReal(recent_list_amount) > IntType(0))
		{
			...
			draw_label(self, { text_get({ /*"startuprecentprojects"*/ STR(3932) }), sVar(dx), sVar(dy) + IntType(16), fa_left, fa_middle, global::c_accent, IntType(1) });
			IntType labelwid = string_width(text_get({ /*"startuprecentprojects"*/ STR(3932) }));
	...
	{
		global::_app->world_import_settings_popup = new_popup({ /*"worldsettings"*/ STR(3933), ID_popup_worldsettings_draw, IntType(600), IntType(532), true, false, false, true, ID_world_import_apply_settings });
		...
		sortlist_column_add(global::_app->world_import_settings_block_list, /*"blockfilter"*/ STR(2925), IntType(0));
		...
		sortlist_column_add(global::_app->world_import_settings_filter_list, /*"blockfilter"*/ STR(2925), IntType(0));
	...
	{
		sStr(window_state) = /*"world_import"*/ STR(2692);
		sStr(popup_ani_type) = /*""*/ STR(0);
		sStr(world_import_world_root) = /*""*/ STR(0);
		sVar(world_import_world_name) = text_get({ /*"worldimportnoworld"*/ STR(3934) });
		sStr(world_import_dimension) = /*"overworld"*/ STR(3935);
	...
	{
		menu_add_item({ /*"overworld"*/ STR(3935), text_get({ /*"worldimportoverworld"*/ STR(3945) }) });
		if (world_import_has_dimension(/*"nether"*/ STR(3946)))
			menu_add_item({ /*"nether"*/ STR(3946), text_get({ /*"worldimportnether"*/ STR(3947) }) });
		if (world_import_has_dimension(/*"end"*/ STR(3948)))
			menu_add_item({ /*"end"*/ STR(3948), text_get({ /*"worldimportend"*/ STR(3949) }) });
	}
	...
	void window_draw_world_import(ScopeAny self)
	{
		...
		if (hasselection)
		{
			...
			confirmw = string_width(text_get({ /*"worldimportconfirm"*/ STR(3950) })) + IntType(70);
		}
		else
			confirmw = IntType(0);
		...
		sBool(content_mouseon) = true;
		draw_set_font(sInt(font_button));
		sVar(dw) = string_width(text_get({ /*"worldimportcancel"*/ STR(3951) })) + IntType(24);
		if (draw_button_label(self, /*"worldimportcancel"*/ STR(3951), sVar(dx), sVar(content_y) + IntType(4), null_, null_, e_button_SECONDARY, null_, e_anchor_LEFT))
			world_import_cancel();
		sVar(dx) += sVar(dw);
		sVar(dx) += IntType(12);
		...
		sVar(dw) = IntType(208);
		capwid = IntType(80);
		draw_button_menu(self, { /*"worldimportdimension"*/ STR(1473), e_menu_LIST, sVar(dx), sVar(dy), sVar(dw), IntType(24), sStr(world_import_dimension), text_get({ /*"worldimport"*/ STR(2917) + sStr(world_import_dimension) }), ID_world_import_select_dimension, false, null_, null_, /*""*/ STR(0), null_, null_, capwid });
		sVar(dx) += sVar(dw);
		sVar(dx) += IntType(12);
		draw_divide_vertical(sVar(dx), sVar(content_y) + IntType(6), sVar(content_height) - IntType(12));
		sVar(dx) += IntType(12);
		sVar(dw) = IntType(24);
		spacing = IntType(4);
		if (draw_button_icon(self, { /*"worldimportbrowse"*/ STR(3952), sVar(dx), sVar(dy), sVar(dw), sVar(dw), false, icons_FOLDER, null_, false, /*"worldimportbrowsetip"*/ STR(3953) }))
		{
			StringType leveldat = file_dialog_open(text_get({ /*"worldimportbrowseworlds"*/ STR(3954) }) + /*" (level.dat)|level.dat;"*/ STR(3955), /*""*/ STR(0), world_import_get_saves_dir(), text_get({ /*"worldimportbrowsecaption"*/ STR(3956) }));
			if (file_exists_lib(leveldat))
				world_import_select_world(self, filename_dir(leveldat));
		}
		sVar(dx) += sVar(dw) + spacing;
		if (draw_button_icon(self, { /*"worldimportsettings"*/ STR(3957), sVar(dx), sVar(dy), sVar(dw), sVar(dw), false, icons_SETTINGS, null_, false, /*"worldimportsettingstip"*/ STR(3958) }))
			popup_show(self, sInt(world_import_settings_popup));
		sVar(dx) += sVar(dw) + spacing;
		BoolType worldpicked = sStr(world_import_world_root) != /*""*/ STR(0);
		if (draw_button_icon(self, { /*"worldimportreload"*/ STR(3959), sVar(dx), sVar(dy), sVar(dw), sVar(dw), false, icons_REFRESH, null_, !worldpicked, /*"worldimportreloadtip"*/ STR(3960) }))
			world_import_select_world(self, sStr(world_import_world_root), sStr(world_import_dimension));
		sVar(dx) += sVar(dw) + spacing;
		if (draw_button_icon(self, { /*"worldimportgotoplayer"*/ STR(3961), sVar(dx), sVar(dy), sVar(dw), sVar(dw), false, icons_PATH_POINT, null_, !worldpicked, /*"worldimportgotoplayertip"*/ STR(3962) }))
		...
		spacing = IntType(12);
		sVar(dw) = string_width(text_get({ /*"worldimportselection"*/ STR(2920) }) + /*":"*/ STR(760));
		draw_label(self, { text_get({ /*"worldimportselection"*/ STR(2920) }) + /*":"*/ STR(760), sVar(dx), sVar(content_y) + (RealType)sVar(content_height) / 2.0, fa_left, fa_middle, global::c_text_secondary, global::a_text_secondary });
		sVar(dx) += sVar(dw) + spacing;
		draw_set_font(sInt(font_button));
		sVar(dw) = string_width(text_get({ /*"worldimportselectionsmall"*/ STR(3963) })) + IntType(24);
		if (draw_button_label(self, /*"worldimportselectionsmall"*/ STR(3963), sVar(dx), sVar(dy), null_, null_, e_button_SECONDARY, null_, e_anchor_LEFT, !worldpicked))
			world_import_set_selection(/*"small"*/ STR(3964));
		sVar(dx) += sVar(dw) + spacing;
		sVar(dw) = string_width(text_get({ /*"worldimportselectionmedium"*/ STR(3965) })) + IntType(24);
		if (draw_button_label(self, /*"worldimportselectionmedium"*/ STR(3965), sVar(dx), sVar(dy), null_, null_, e_button_SECONDARY, null_, e_anchor_LEFT, !worldpicked))
			world_import_set_selection(/*"medium"*/ STR(599));
		sVar(dx) += sVar(dw) + spacing;
		sVar(dw) = string_width(text_get({ /*"worldimportselectionlarge"*/ STR(3966) })) + IntType(24);
		if (draw_button_label(self, /*"worldimportselectionlarge"*/ STR(3966), sVar(dx), sVar(dy), null_, null_, e_button_SECONDARY, null_, e_anchor_LEFT, !worldpicked))
			world_import_set_selection(/*"large"*/ STR(3967));
		if (world_import_has_selection())
		{
			sVar(dx) += sVar(dw) + IntType(20);
			VecType size = world_import_get_selection_size();
			draw_label(self, { text_get({ /*"worldimportblocks"*/ STR(3968), size.Real(X_), size.Real(Y_), size.Real(Z_) }), sVar(dx), sVar(content_y) + (RealType)sVar(content_height) / 2.0, fa_left, fa_middle, global::c_text_main, global::a_text_main, sInt(font_value) });
		}
		sVar(content_x) = IntType(0);
		sVar(content_y) = IntType(0);
		sVar(content_width) = sReal(window_width);
		sVar(content_height) = sReal(window_height);
		if (hasselection)
			if (draw_button_label(self, /*"worldimportconfirm"*/ STR(3950), confirmx, confirmy, confirmw, null_, e_button_BIG, null_, e_anchor_LEFT))
		...
		{
			VarType filtertext = text_get({ /*"worldimportfilteractive"*/ STR(3969) });
	...
	void popup_worldsettings_draw(ScopeAny self)
	{
		...
		draw_switch(self, /*"worldsettingsunloadregions"*/ STR(3970), sVar(dx), sVar(dy), sVar(setting_world_import_unload_regions), ID_action_world_import_settings_unload_regions, /*"worldsettingsunloadregionstip"*/ STR(3971));
		tab_next(self);
		tab_control_switch(self);
		draw_switch(self, /*"worldsettingsfilterenabled"*/ STR(3972), sVar(dx), sVar(dy), sVar(setting_world_import_filter_enabled), ID_action_world_import_settings_filter_enabled);
		tab_next(self);
		if (sVar(setting_world_import_filter_enabled) > 0)
		{
			tab_control_togglebutton(self);
			togglebutton_add(self, { /*"worldsettingsfilterremove"*/ STR(3973), null_, IntType(0), sVar(setting_world_import_filter_mode) == IntType(0), ID_action_world_import_settings_filter_mode });
			togglebutton_add(self, { /*"worldsettingsfilterkeep"*/ STR(3974), null_, IntType(1), sVar(setting_world_import_filter_mode) == IntType(1), ID_action_world_import_settings_filter_mode });
			draw_togglebutton(self, /*"worldsettingsfiltermode"*/ STR(3975), sVar(dx), sVar(dy));
			tab_next(self);
			RealType listdw = sVar(content_width) - IntType(24);
			sVar(dy) += IntType(8);
			tab_control_sortlist(self, IntType(10));
			sortlist_draw(self, sInt(world_import_settings_block_list), sVar(dx), sVar(dy), listdw / 2.0 - IntType(20), sReal(tab_control_h), sInt(world_import_settings_block_select), false, text_get({ /*"worldsettingsfilterblocks"*/ STR(3976) }));
			sortlist_draw(self, sInt(world_import_settings_filter_list), sVar(dx) + listdw / 2.0 + IntType(20), sVar(dy), listdw / 2.0 - IntType(20), sReal(tab_control_h), sInt(world_import_settings_filter_select), false, text_get({ /*"worldsettingsfilterfiltered"*/ STR(3977) }));
			if (draw_button_icon(self, { /*"worldsettingsfilterright"*/ STR(3978), sVar(dx) + listdw / 2.0 - IntType(12), sVar(dy) + (sReal(tab_control_h) / 2.0) - IntType(16), IntType(24), IntType(24), false, icons_CHEVRON_RIGHT_TINY, null_, sInt(world_import_settings_block_select) == null_ }))
			{
			...
			if (draw_button_icon(self, { /*"worldsettingsfilterleft"*/ STR(3979), sVar(dx) + listdw / 2.0 - IntType(12), sVar(dy) + (sReal(tab_control_h) / 2.0) + IntType(16), IntType(24), IntType(24), false, icons_CHEVRON_LEFT_TINY, null_, sInt(world_import_settings_filter_select) == null_ }))
			{
			...
			sVar(dy) += IntType(8);
			draw_tooltip_label(self, /*"worldsettingsfilterhelp"*/ STR(3980), icons_INFO, e_toast_INFO);
		}
	}

	StringType yesno(RealType val)
	{
		return ((val > 0) ? /*"yes"*/ STR(3981) : /*"no"*/ STR(3982));
	}
