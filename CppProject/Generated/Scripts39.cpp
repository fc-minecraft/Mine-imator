...
namespace CppProject
{
	...
	IntType new_biome(StringType name, RealType tx, RealType ty, BoolType hardcoded, IntType grass, IntType foliage, IntType water, IntType base)
	{
		withOne (obj_biome, (new obj_biome)->id, noone)
		{
			...
			self->group = false;
			self->display_name = minecraft_asset_get_name(/*"biome"*/ STR(1656), name);
	...
	{
		StringType charlist = /*" !\"#$%&'()*+,-./0123456789:;<=>?@ABCDEFGHIJKLMNOPQRSTUVWXYZ[\\]^_`abcdefghijklmnopqrstuvwxyz{|}~¿ÀÁÂÃÄÅÆÇÈÉÊËÌÍÎÏÐÑÒÓÔÕÖ×ØÙÚÛÜÝÞßàáâãäåæçèéêëìíîïðñòóôõö÷øùúûüýþÿỳ"*/ STR(1887);
	...
	IntType new_minecraft_map(StringType fn)
	{
		...
		{
			log({ /*"Minecraft map error"*/ STR(1889), /*"Not a map file"*/ STR(1890) });
			return IntType(0);
		}
		data = DsMap(rootmap).Value(/*""*/ STR(0));
		data = DsMap(data).Value(/*"data"*/ STR(1722));
		if (is_undefined(data))
		{
			log({ /*"Minecraft map error"*/ STR(1889), /*"Data not available"*/ STR(1891) });
			return IntType(0);
		}
		colors = DsMap(data).Value(/*"colors"*/ STR(1700));
		if (is_undefined(colors))
		{
			log({ /*"Minecraft map error"*/ STR(1889), /*"Colors not found"*/ STR(1892) });
			return IntType(0);
		}
	...
	IntType new_res(ScopeAny self, VarType fn, VarType type)
	{
		...
		res = null_;
		replaced = false;
		newfn = sStr(project_folder) + /*"/"*/ STR(19) + filename_name(fn);
		...
		if (res != null_)
		{
			if (question(text_get({ /*"questionreplace"*/ STR(1893) })))
				replaced = true;
			else
			{
				res = null_;
		...
		log({ /*"Add resource"*/ STR(1894), DsList(global::res_type_name_list).Value(type) });
		log({ /*"filename"*/ STR(1243), ObjType(obj_resource, res)->filename });
		return res;
	}
	...
	IntType new_shader(StringType name)
	{
		withOne (obj_shader, (new obj_shader)->id, noone)
		{
			...
			self->script = asset_get_index(name + /*"_set"*/ STR(1895));
			self->uniform_map = ds_string_map_create();
			self->sampler_map = ds_string_map_create();
			new_shader_sampler(self, /*"uTexture"*/ STR(1896));
			new_shader_uniform(self, /*"uTextureSize"*/ STR(1897));
			new_shader_uniform(self, /*"uBlendColor"*/ STR(1898));
			new_shader_uniform(self, /*"uTime"*/ STR(1899));
			new_shader_uniform(self, /*"uWindEnable"*/ STR(1900));
			new_shader_uniform(self, /*"uWindTerrain"*/ STR(1901));
			new_shader_uniform(self, /*"uWindSpeed"*/ STR(1902));
			new_shader_uniform(self, /*"uWindStrength"*/ STR(1903));
			new_shader_uniform(self, /*"uWindDirection"*/ STR(1904));
			new_shader_uniform(self, /*"uWindDirectionalSpeed"*/ STR(1905));
			new_shader_uniform(self, /*"uWindDirectionalStrength"*/ STR(1906));
			new_shader_uniform(self, /*"uFogShow"*/ STR(1907));
			new_shader_uniform(self, /*"uFogColor"*/ STR(1908));
			new_shader_uniform(self, /*"uFogDistance"*/ STR(1909));
			new_shader_uniform(self, /*"uFogSize"*/ STR(1910));
			new_shader_uniform(self, /*"uFogHeight"*/ STR(1911));
			new_shader_uniform(self, /*"uCameraPosition"*/ STR(1912));
			new_shader_uniform(self, /*"uTAAMatrix"*/ STR(1913));
			new_shader_uniform(self, /*"uSampleIndex"*/ STR(1914));
			new_shader_uniform(self, /*"uAlphaHash"*/ STR(1915));
	...
	IntType new_tab(StringType location, BoolType show, IntType header, IntType window)
	{
		...
		location = string_replace(location, /*"_top"*/ STR(1916), /*""*/ STR(0));
		location = string_replace(location, /*"_bottom"*/ STR(1917), /*"_secondary"*/ STR(1918));
	...
	{
		return new_textbox(true, IntType(64), /*".0123456789"*/ STR(1919) + /*"+-/*^()"*/ STR(1920));
	}
	
	IntType new_textbox_hex()
	{
		return new_textbox(true, IntType(6), /*"0123456789ABCDEFabcdef"*/ STR(1921));
	}
	
	IntType new_textbox_integer()
	{
		return new_textbox(true, IntType(64), /*"0123456789"*/ STR(1025) + /*"+-/*^()"*/ STR(1920));
	}
	
	IntType new_textbox_ndecimals()
	{
		return new_textbox(true, IntType(64), /*"-.0123456789"*/ STR(1922) + /*"+-/*^()"*/ STR(1920));
	}
	
	IntType new_textbox_ninteger()
	{
		return new_textbox(true, IntType(64), /*" - 0123456789"*/ STR(1923) + /*"+-/*^()"*/ STR(1920));
	}
	...
	IntType new_transition_texture_map(RealType w, RealType h, RealType padding, BoolType center)
	{
			...
			surface_set_target(surf);
		{
			...
			for (RealType d = IntType(0); d <= quality; d++)
			{
				...
				if (DsList(global::transition_list).Value(t) == /*"bezier"*/ STR(551))
				{
	...
	RealType object_save(ScopeAny self)
	{
		...
		if (fn == /*""*/ STR(0))
			return IntType(0);
		fn = filename_new_ext(fn, /*".miobject"*/ STR(686));
		log({ /*"Saving object"*/ STR(1924), fn });
	...
	RealType open_url(ScopeAny self, StringType url)
	{
		if (url == /*""*/ STR(0))
			return IntType(0);
		if (sVar(os_type) == sVar(os_windows) || sVar(os_type) == sVar(os_macosx) || sVar(os_type) == sVar(os_linux))
			lib_open_url(url);
		else
			url_open(url);
	...
	void panel_area_draw(ScopeAny self)
	{
		...
		withAll (obj_panel, global::_app->id)
		{
			...
			self->size_real_ani = ceil((self->size_real - IntType(16)) + (IntType(16) * ease(/*"easeoutcirc"*/ STR(571), self->size_ani)));
			self->glow_ani += test_reduced_motion(IntType(1), 0.035 * global::delta);
			self->glow_ani = clamp(self->glow_ani, IntType(0), IntType(1));
			self->size_glow = (self->size - IntType(16)) + (IntType(16) * ease(/*"easeoutcirc"*/ STR(571), self->glow_ani));
		...
		if (sVar(window_busy) == /*"panelresize"*/ STR(1926))
		{
		...
		if (sVar(window_busy) == /*"tabmove"*/ STR(1927))
		{
	...
	RealType panel_draw(ScopeAny self, IntType panel)
	{
		...
		if (idBool(sVar(content_tab), movable))
		{
			...
			for (IntType t = IntType(0); t < ObjType(obj_panel, panel)->tab_list_amount; t++)
			{
				...
				if (idBool(tab, closeable) && (hover > 0 || sel > 0))
				{
					...
					if (draw_button_icon(self, { /*"tabclose"*/ STR(1928) + string(tab), floor(dx + dw - IntType(20)), dy + IntType(4), IntType(16), IntType(16), false, icons_CLOSE_SMALL }))
					{
						tab_close(self, tab);
						return IntType(0);
					}
				...
				if (sVar(window_busy) == /*"tabmove"*/ STR(1927))
				{
					...
					sVar(window_busy) = /*"tabmove"*/ STR(1927);
				}
		...
		{
			sVar(window_busy) = /*"panelresize"*/ STR(1926);
		...
		{
			sVar(window_busy) = /*"tabclick"*/ STR(1929);
			sVar(tab_move) = sVar(content_tab);
		}
		if (sVar(window_busy) == /*"tabclick"*/ STR(1929))
		{
