...
namespace CppProject
{
	RealType block_tile_entity_skull(Scope<obj_builder_thread> self, VarType map)
	{
		...
		VarType texlist;
		mapname = /*"Owner"*/ STR(956);
		if (!ds_map_valid(DsMap(map).Value(mapname)))
		{
			mapname = /*"SkullOwner"*/ STR(957);
			if (!ds_map_valid(DsMap(map).Value(mapname)))
				return IntType(0);
		}
		map = DsMap(map).Value(mapname);
		ownerid = string(DsMap(map).Value(/*"Id"*/ STR(958)));
		map = DsMap(map).Value(/*"Properties"*/ STR(959));
		if (!ds_map_valid(map))
			return IntType(0);
		texlist = DsMap(map).Value(/*"textures"*/ STR(814));
		if (!ds_list_valid(texlist))
			return IntType(0);
		map = DsList(texlist).Value(IntType(0));
		texvalue = base64_decode(DsMap(map).Value(/*"Value"*/ STR(960)));
		map = json_decode(texvalue);
		if (!ds_map_valid(map))
			return IntType(0);
		if (!ds_map_valid(DsMap(map).Value(/*"textures"*/ STR(814))))
			return IntType(0);
		map = DsMap(map).Value(/*"textures"*/ STR(814));
		map = DsMap(map).Value(/*"SKIN"*/ STR(961));
		texurl = DsMap(map).Value(/*"url"*/ STR(962)).Str();
	...
	RealType builder_generate(ScopeAny self)
	{
		...
		if (builder_get_waterlogged(Scope<obj_builder_thread>(self), sInt(build_pos_x), sInt(build_pos_y), sInt(build_pos_z)))
		{
			...
			sInt(block_pos_y) = sInt(build_pos_y) * block_size_;
			sInt(block_pos_z) = sInt(build_pos_z) * block_size_;
			sInt(block_current) = DsMap(ObjType(obj_minecraft_assets, global::mc_assets)->block_name_map).Value(/*"water"*/ STR(789));
	...
	BoolType builder_read_schematic(Scope<obj_builder> self, VarType map)
	{
		if (!ds_map_valid(map))
		{
			log({ /*"Schematic error"*/ STR(963), /*"Not a schematic file"*/ STR(964) });
			return false;
		}
		self->builder_scenery_legacy = is_undefined(DsMap(map).Value(/*"Palette"*/ STR(965)));
		self->build_size_x = DsMap(map).Value(/*"Width"*/ STR(966));
		self->build_size_y = DsMap(map).Value(/*"Length"*/ STR(967));
		self->build_size_z = DsMap(map).Value(/*"Height"*/ STR(968));
		log({ /*"Size"*/ STR(59), string(self->build_size_x) + /*" x "*/ STR(969) + string(self->build_size_y) + /*" x "*/ STR(969) + string(self->build_size_z) });
		if (is_undefined(self->build_size_x) || is_undefined(self->build_size_y) || is_undefined(self->build_size_z))
		{
			log({ /*"Schematic error"*/ STR(963), /*"Size not fully defined"*/ STR(970) });
			return false;
		}
		if (self->build_size_x <= IntType(0) || self->build_size_y <= IntType(0) || self->build_size_z <= IntType(0))
		{
			log({ /*"Schematic error"*/ STR(963), /*"Size cannot be 0"*/ STR(971) });
			return false;
		}
		if (!self->builder_scenery_legacy)
		{
			VarType version = DsMap(map).Value(/*"Version"*/ STR(972));
			if (is_undefined(version))
			{
				log({ /*"Schematic error"*/ STR(963), /*"Version not available"*/ STR(973) });
				return false;
			}
			log({ /*"Version"*/ STR(972), DsMap(map).Value(/*"Version"*/ STR(972)) });
			if (version > IntType(1))
			{
				log({ /*"Schematic error"*/ STR(963), /*"Unsupported format, version too high"*/ STR(974) });
				return false;
			}
			VarType palettemap = DsMap(map).Value(/*"Palette"*/ STR(965));
			if (!ds_map_valid(palettemap))
			{
				log({ /*"Schematic error"*/ STR(963), /*"Palette not found"*/ STR(975) });
				return false;
			}
			...
			while (!is_undefined(key))
			{
				if (!string_contains(key, /*"_NBT_"*/ STR(976)))
				{
					VarType index;
					IntType bracketindex;
					index = DsMap(palettemap).Value(key);
					bracketindex = string_pos(/*"["*/ STR(977), key);
					if (bracketindex > IntType(0))
					{
						...
						if (!is_undefined(block))
						{
							...
							if (state_vars_get_value(vars, /*"waterlogged"*/ STR(807)) != /*"false"*/ STR(749))
								if (ObjType(obj_block, block)->waterlogged > 0 || state_vars_get_value(vars, /*"waterlogged"*/ STR(807)) == /*"true"*/ STR(748))
			...
			self->sch_blockdata_array = DsMap(map).Value(/*"BlockData"*/ STR(978));
			if (is_undefined(self->sch_blockdata_array))
			{
				log({ /*"Schematic error"*/ STR(963), /*"BlockData array not found"*/ STR(979) });
				return false;
			}
			self->sch_blockdata_ints = (DsMap(map).Value(/*"BlockData_NBT_type"*/ STR(980)) == e_nbt_TAG_INT_ARRAY);
			debug({ /*"blockdataints"*/ STR(981), self->sch_blockdata_ints });
			VarType metadata = DsMap(map).Value(/*"Metadata"*/ STR(982));
			if (ds_map_valid(metadata))
				self->file_map = DsMap(metadata).Value(/*"FromMap"*/ STR(983));
		}
		else
		{
			self->sch_legacy_blocksarray = DsMap(map).Value(/*"Blocks"*/ STR(984));
			if (is_undefined(self->sch_legacy_blocksarray))
			{
				log({ /*"Schematic error"*/ STR(963), /*"Blocks array not found"*/ STR(985) });
				return false;
			}
			self->sch_legacy_dataarray = DsMap(map).Value(/*"Data"*/ STR(986));
			if (is_undefined(self->sch_legacy_dataarray))
			{
				log({ /*"Schematic error"*/ STR(963), /*"Data array not found"*/ STR(987) });
				return false;
			}
			self->file_map = DsMap(map).Value(/*"FromMap"*/ STR(983));
		}
		
		if (is_undefined(self->file_map))
			self->file_map = /*""*/ STR(0);
		self->sch_tileentity_list = DsMap(map).Value(/*"TileEntities"*/ STR(988));
		return true;
	}
	...
	BoolType builder_read_schematic_nbt(ScopeAny self, VarType structuremap)
	{
		...
		if (!is_undefined(DsMap(structuremap).Value(/*"DataVersion"*/ STR(989))))
			structureversion = DsMap(structuremap).Value(/*"DataVersion"*/ STR(989));
		if (structureversion < IntType(2000))
		{
			log({ /*"Structure error"*/ STR(990), /*"Unsupported format, version too low"*/ STR(991) });
			return false;
		}
		sizemap = DsMap(structuremap).Value(/*"size"*/ STR(992));
		...
		{
			log({ /*"Structure error"*/ STR(990), /*"Size cannot be 0"*/ STR(971) });
			return false;
		}
		VarType paletteslist, palettelist;
		paletteslist = DsMap(structuremap).Value(/*"palettes"*/ STR(993));
		...
		{
			palettelist = DsMap(structuremap).Value(/*"palette"*/ STR(994));
			if (!ds_list_valid(palettelist))
			{
				log({ /*"Structure error"*/ STR(990), /*"Palette not found"*/ STR(975) });
				return false;
			}
		}
		...
		for (IntType i = IntType(0); i < ds_list_size((IntType)(palettelist)); i++)
		{
			...
			mcid = DsMap(blockmap).Value(/*"Name"*/ STR(995));
			...
			if (!is_undefined(block))
			{
				...
				if (ObjType(obj_block, block)->id_state_vars_map != null_ && is_array(DsMap(ObjType(obj_block, block)->id_state_vars_map).Value(mcid)))
					state_vars_add(VarType::CreateRef(vars), DsMap(ObjType(obj_block, block)->id_state_vars_map).Value(mcid));
				propertiesmap = DsMap(blockmap).Value(/*"Properties"*/ STR(959));
		...
		VarType blocklist = DsMap(structuremap).Value(/*"blocks"*/ STR(996));
		if (!ds_list_valid(blocklist))
		{
			log({ /*"Structure error"*/ STR(990), /*"Block list not found"*/ STR(997) });
			return false;
		}
		withOne (obj_builder, global::mc_builder, self->id)
		{
			...
			for (IntType i = IntType(0); i < ds_list_size((IntType)(blocklist)); i++)
			{
				...
				pos = DsMap(blockmap).Value(/*"pos"*/ STR(998));
				state = DsMap(blockmap).Value(/*"state"*/ STR(999));
				...
				blocknbt = DsMap(blockmap).Value(/*"nbt"*/ STR(1000));
				if (!is_undefined(blocknbt))
				{
					VarType finalstate, script;
					finalstate = DsMap(blocknbt).Value(/*"final_state"*/ STR(1001));
				...
				if (entity != null_)
				{
					self->script = asset_get_index(/*"block_tile_entity_"*/ STR(1002) + string_replace(string_lower(entity), /*"minecraft:"*/ STR(810), /*""*/ STR(0)));
		...
		debug_timer_stop(/*"res_load_scenery, Parse blocks"*/ STR(1003));
		return true;
	}
		...
		if (ds_list_valid(self->sch_tileentity_list))
		{
			...
			withOne (obj_builder_thread, DsList(self->thread_list).Value(IntType(0)), self->id)
			{
				...
				for (IntType i = IntType(0); i < ds_list_size((IntType)(ObjType(obj_builder, self.otherId)->sch_tileentity_list)); i++)
				{
					...
					if (!self->builder_scenery_legacy)
					{
						eid = DsMap(entity).Value(/*"Id"*/ STR(958));
						VarType poslist = DsMap(entity).Value(/*"Pos"*/ STR(1005));
					...
					else
					{
						...
						ex = DsMap(entity).Value(/*"x"*/ STR(822));
						ey = DsMap(entity).Value(/*"z"*/ STR(823));
						ez = DsMap(entity).Value(/*"y"*/ STR(805));
					}
					
					if (is_string(eid))
					{
						VarType script = asset_get_index(/*"block_tile_entity_"*/ STR(1002) + string_replace(string_lower(eid), /*"minecraft:"*/ STR(810), /*""*/ STR(0)));
						if (script > -IntType(1))
						{
							self->build_pos_x = ex;
							self->build_pos_y = ey;
				...
				debug_timer_stop(/*"Parse Tile Entities"*/ STR(1006));
			}
