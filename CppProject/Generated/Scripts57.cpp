...
namespace CppProject
{
	RealType sortlist_draw(ScopeAny self, IntType slist, VarType xx, VarType yy, VarType w, RealType h, VarType select, BoolType filter, VarType name)
	{
		...
		{
			if (draw_button_icon(self, { /*"listfilter"*/ STR(2928) + string(slist), xx, yy, IntType(24), IntType(24), !ds_list_empty(idInt(slist, filter_list)), icons_FILTER, null_, false, /*"tooltipfilterlist"*/ STR(2929) }))
			{
				menu_settings_set(self, xx, yy, /*"listfilter"*/ STR(2928) + string(slist), IntType(24));
				sInt(settings_menu_script) = ID_sortlist_filters_draw;
				sVar(settings_menu_sortlist) = slist;
				sVar(settings_menu_h_max) = IntType(256);
			}
			if ((sStr(settings_menu_name) == /*"listfilter"*/ STR(2928) + string(slist)) && sStr(settings_menu_ani_type) != /*"hide"*/ STR(722))
		...
		if (draw_textfield(self, /*"listsearch"*/ STR(2930) + string(slist), searchx, yy, searchw, IntType(24), idInt(slist, search_tbx), null_, text_get({ /*"listsearch"*/ STR(2930) }), /*"none"*/ STR(880)))
		{
		...
		if (sVar(window_busy) == /*"sortlist_resize"*/ STR(2931) && sInt(sortlist_resize) == slist)
		{
		...
		for (IntType c = IntType(0); c < idInt(slist, columns); c++)
		{
			...
			if (c > IntType(0) && app_mouse_box(self, xx + dx - IntType(5), yy, IntType(10), colsh) && sBool(content_mouseon))
			{
				...
				if (sBool(mouse_left_pressed))
				{
					...
					sVar(window_busy) = /*"sortlist_resize"*/ STR(2931);
				}
			}
			icon = null_;
			if (idInt(slist, column_sort) == c)
				icon = ((idReal(slist, sort_asc) > 0) ? icons_SORT_UP : icons_SORT_DOWN);
			if (sortlist_draw_button(self, /*"column"*/ STR(2932) + idArr(slist, column_name).Value(c), xx + dx, yy + IntType(4), idArr(slist, column_w).Value(c), colsh, idInt(slist, column_sort) == c, icon, (c == IntType(0)), (c == idInt(slist, columns) - IntType(1)), colmouseon))
			{
				if (idInt(slist, column_sort) == c)
				{
		...
		if (sVar(window_focus) == string(idInt(slist, scroll)))
		{
			...
			if (!app_mouse_box(self, xx, yy, w, h) && sBool(content_mouseon) && sBool(mouse_left) && sVar(window_busy) != /*"scrollbar"*/ STR(2592))
	...
	RealType sortlist_filters_draw(ScopeAny self)
	{
		...
		for (IntType i = IntType(0); i < ds_list_size(typelist); i++)
		{
			...
			capwid = max({ capwid, string_width(text_get({ /*"type"*/ STR(777) + itemname })) });
			...
			if (draw_checkbox(self, /*"type"*/ STR(777) + itemname, sVar(dx), sVar(dy) + scroll, active, null_))
			{
				if (active)
					...
					for (IntType i = IntType(0); i < array_length(VarType::CreateRef(ObjType(obj_block, block)->default_state)) && !match; i += IntType(2))
					{
						...
						for (IntType s = IntType(0); s < idReal(statecurrent, value_amount) && !match; s++)
							if (string_count(check, string_lower(minecraft_asset_get_name(/*"blockstatevalue"*/ STR(5), idVar(statecurrent, value_name).Value(s)))) > IntType(0))
								match = true;
						if (string_count(check, string_lower(text_get({ /*"blockstate"*/ STR(768) + state }))) > IntType(0))
							match = true;
					}
						...
						for (IntType i = IntType(0); i < array_length(VarType::CreateRef(ObjType(obj_model, model)->default_state)) && !match; i += IntType(2))
						{
							...
							for (IntType s = IntType(0); s < idReal(statecurrent, value_amount) && !match; s++)
								if (string_count(check, string_lower(minecraft_asset_get_name(/*"modelstatevalue"*/ STR(9), idVar(statecurrent, value_name).Value(s)))) > IntType(0))
									match = true;
							if (string_count(check, string_lower(text_get({ /*"modelstate"*/ STR(747) + state }))) > IntType(0))
								match = true;
						}
	...
	{
		return string_replace_all(string_upper(string_char_at(str, IntType(1))) + string_delete(str, IntType(1), IntType(1)), /*"_"*/ STR(1110), /*" "*/ STR(20));
	}
	...
	VarType string_get_real(VarType str, VarType inv)
	{
		...
		while (len > IntType(1) && string_char_at(str, len) == /*" "*/ STR(20))
			len--;
		...
		if (string_replace(string_replace(str, /*"-"*/ STR(1089), /*""*/ STR(0)), /*"."*/ STR(1087), /*""*/ STR(0)) == /*""*/ STR(0))
			return inv;
		if (string_contains(str, /*"-."*/ STR(2933)))
			str = string_replace(str, /*"-."*/ STR(2933), /*"-0."*/ STR(2934));
		if (state >= IntType(2))
			return real(str);
		return inv;
	}
	...
	StringType string_limit_ext(VarType str, VarType w, VarType h)
	{
		...
		nstr = /*""*/ STR(0);
		lh = string_height(/*" "*/ STR(20));
		dx = IntType(0);
		dy = IntType(0);
		if (lh > h || string_width(/*"..."*/ STR(2936)) > w)
			return /*""*/ STR(0);
		linestart = IntType(1);
		wordstart = IntType(1);
		for (c = IntType(1); c <= string_length(str); c++)
		{
			...
			if (char_ == /*"\n"*/ STR(945))
			{
				...
				if (dy + lh > h)
				{
					while (c > linestart && string_width(string_copy(str, linestart, (IntType)(c - linestart)) + /*"..."*/ STR(2936)) > w)
						c--;
					
					nstr += string_copy(str, linestart, (IntType)(c - linestart)) + /*"..."*/ STR(2936);
					c = linestart;
					break;
				}
			...
			{
				if (char_ == /*" "*/ STR(20) || char_ == /*", "*/ STR(1029) || char_ == /*"."*/ STR(1087) || char_ == /*"/"*/ STR(19) || char_ == /*"-"*/ STR(1089))
	...
	ArrType string_split(VarType str, StringType sep)
	{
		...
		while (true)
		{
			...
			if (pos == IntType(0))
				break;
			IntType escapepos = string_pos(/*"\\"*/ STR(1189) + sep, str);
	...
	{
		StringType sep = /*":"*/ STR(760);
		return string_repeat(/*"0"*/ STR(1050), (IntType)(hours < IntType(10))) + string(hours) + sep + string_repeat(/*"0"*/ STR(1050), (IntType)(mnts < IntType(10))) + string(mnts) + sep + string_repeat(/*"0"*/ STR(1050), (IntType)(secs < IntType(10))) + string_replace_all(string_format(secs, IntType(2), IntType(3)), /*" "*/ STR(20), /*""*/ STR(0));
	}
