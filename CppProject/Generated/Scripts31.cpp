...
namespace CppProject
{
	...
	{
		return file_dialog_save(text_get({ /*"filedialogsaveobject"*/ STR(1163) }) + /*" (*.miobject)|*.miobject"*/ STR(1164), filename_get_valid(fn), /*""*/ STR(0), text_get({ /*"filedialogsaveobjectcaption"*/ STR(1165) }));
	}
	
	StringType file_dialog_save_particles(VarType fn)
	{
		return file_dialog_save(text_get({ /*"filedialogsaveparticles"*/ STR(1166) }) + /*" (*.miparticles)|*.miparticles"*/ STR(1167), filename_get_valid(fn), particles_directory, text_get({ /*"filedialogsaveparticlescaption"*/ STR(1168) }));
	}
	
	StringType file_dialog_save_project(ScopeAny self, VarType fn)
	{
		return file_dialog_save(/*""*/ STR(0), fn, sStr(setting_project_folder), text_get({ /*"filedialogsaveprojectcaption"*/ STR(1169) }));
	}
	
	StringType file_dialog_save_render(StringType fn)
	{
		return file_dialog_save(text_get({ /*"filedialogsaverender"*/ STR(1170) }) + /*" (*.mirender)|*.mirender"*/ STR(1171), filename_get_valid(fn), render_directory, text_get({ /*"filedialogsaverendercaption"*/ STR(1172) }));
	}
	
	StringType file_dialog_save_resource(VarType fn, StringType exts)
	{
		return file_dialog_save(/*" * "*/ STR(675) + exts + /*"|*"*/ STR(1173) + exts, filename_get_valid(fn), /*""*/ STR(0), text_get({ /*"filedialogsaveresourcecaption"*/ STR(1174) }));
	}
	...
	{
		log({ /*"working_directory"*/ STR(1175), gmlGlobal::working_directory });
		if (!directory_exists_lib(gmlGlobal::working_directory))
		{
			log({ /*"directory broken"*/ STR(1176) });
			access_error();
			return false;
		}
		IntType surf;
		...
		tmpfile1 = file_directory + /*"tmp.png"*/ STR(1177);
		tmpfile2 = data_directory + /*"tmp.png"*/ STR(1177);
		log({ /*"Trying to save files"*/ STR(1178) });
		...
		{
			log({ /*"Couldn't save to file_directory"*/ STR(1179) });
			access_error();
			return false;
		}
		if (!file_exists_lib(tmpfile2))
		{
			log({ /*"Couldn't save to data_directory"*/ STR(1180) });
			access_error();
			return false;
		}
		log({ /*"surface_save OK"*/ STR(1181) });
		IntType tex1, tex2;
		...
		{
			log({ /*"Couldn't load texture from file_directory"*/ STR(1182) });
			access_error();
			return false;
		}
		if (tex2 < IntType(0) || texture_width(tex2) != IntType(32) || texture_height(tex2) != IntType(32))
		{
			log({ /*"Couldn't load texture from data_directory"*/ STR(1183) });
			access_error();
			return false;
		}
		log({ /*"texture_create OK"*/ STR(1184) });
		file_delete_lib(tmpfile1);
		file_delete_lib(tmpfile2);
		if (file_exists_lib(tmpfile1))
		{
			log({ /*"Couldn't delete from file_directory"*/ STR(1185) });
			access_error();
			return false;
		}
		if (file_exists_lib(tmpfile1))
		{
			log({ /*"Couldn't delete from data_directory"*/ STR(1186) });
			access_error();
			return false;
		}
		log({ /*"file_delete_lib OK"*/ STR(1187) });
		return true;
	}
	...
	{
		fn = string_replace_all(fn, /*"/"*/ STR(19), /*"_"*/ STR(1110));
		fn = string_replace_all(fn, /*"\\"*/ STR(1189), /*"_"*/ STR(1110));
		fn = string_replace_all(fn, /*":"*/ STR(760), /*"_"*/ STR(1110));
		fn = string_replace_all(fn, /*"*"*/ STR(1091), /*"_"*/ STR(1110));
		fn = string_replace_all(fn, /*"?"*/ STR(1190), /*"_"*/ STR(1110));
		fn = string_replace_all(fn, /*"\""*/ STR(1191), /*"_"*/ STR(1110));
		fn = string_replace_all(fn, /*"<"*/ STR(1192), /*"_"*/ STR(1110));
		fn = string_replace_all(fn, /*">"*/ STR(1193), /*"_"*/ STR(1110));
		fn = string_replace_all(fn, /*"|"*/ STR(856), /*"_"*/ STR(1110));
		while (string_char_at(fn, string_length(fn)) == /*" "*/ STR(20))
	...
	VarType filename_new_ext(VarType fn, StringType newext)
	{
		...
		for (p = string_length(fn); p >= IntType(0); p--)
		{
			...
			if (p == IntType(0) || c == /*"\\"*/ STR(1189) || c == /*"/"*/ STR(19))
				return fn + newext;
			if (c == /*"."*/ STR(1087))
				break;
		}
		...
		for (IntType i = IntType(0); i < self->loaded_amount; i++)
		{
			withOne (Object, save_id_find(self->loaded_save_id.Value(i)), self->id)
			{
				if (self->subAssetId == ID_obj_resource && sBool(copied))
					file_delete_lib(global::_app->project_folder + /*"/"*/ STR(19) + sVar(filename));
